#!/bin/bash
# Rabbitmq Module Integration Test
# Uses Python requests from inside container for reliable HTTP requests

set -e

echo "=========================================="
echo "Rabbitmq Module Integration Test"
echo "=========================================="
...
# Test Rabbitmq webhook
print("")
print("Testing Rabbitmq webhook...")
try:
    response = requests.post(
        f"{webhook_url}/webhook/test_rabbitmq",
        json={"test": "data", "message": "Hello Rabbitmq"},
        headers={
    ...
    if response.status_code == 200:
        print(f"✓ Rabbitmq webhook test passed (HTTP {response.status_code})")
        print(f"Response: {response.json()}")
    else:
        print(f"✗ Rabbitmq webhook test failed (HTTP {response.status_code})")
        print(f"Response: {response.text}")
        sys.exit(1)
except Exception as e:
    print(f"✗ Rabbitmq webhook test failed with error: {e}")
    sys.exit(1)

print("")
print("==========================================")
print("Rabbitmq module tests passed! ✓")
print("==========================================")
EOF
