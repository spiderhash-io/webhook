...
# Test Mysql webhook
print("")
print("Testing Mysql webhook...")
try:
    response = requests.post(
        f"{webhook_url}/webhook/test_mysql",
        json={"test": "data", "message": "Hello Mysql"},
        headers={
    ...
    if response.status_code == 200:
        print(f"✓ Mysql webhook test passed (HTTP {response.status_code})")
        print(f"Response: {response.json()}")
    else:
        print(f"✗ Mysql webhook test failed (HTTP {response.status_code})")
        print(f"Response: {response.text}")
        sys.exit(1)
except Exception as e:
    print(f"✗ Mysql webhook test failed with error: {e}")
    sys.exit(1)

print("")
print("==========================================")
print("Mysql module tests passed! ✓")
print("==========================================")
EOF
