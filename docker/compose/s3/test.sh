...
# Test S3 webhook
print("")
print("Testing S3 webhook...")
try:
    response = requests.post(
        f"{webhook_url}/webhook/test_s3",
        json={"test": "data", "message": "Hello S3"},
        headers={
    ...
    if response.status_code == 200:
        print(f"✓ S3 webhook test passed (HTTP {response.status_code})")
        print(f"Response: {response.json()}")
    else:
        print(f"✗ S3 webhook test failed (HTTP {response.status_code})")
        print(f"Response: {response.text}")
        sys.exit(1)
except Exception as e:
    print(f"✗ S3 webhook test failed with error: {e}")
    sys.exit(1)

print("")
print("==========================================")
print("S3 module tests passed! ✓")
print("==========================================")
EOF
