#!/bin/bash
# Activemq Module Integration Test
# Uses Python requests from inside container for reliable HTTP requests

set -e

echo "=========================================="
echo "Activemq Module Integration Test"
echo "=========================================="
...
# Test Activemq webhook
print("")
print("Testing Activemq webhook...")
try:
    response = requests.post(
        f"{webhook_url}/webhook/test_activemq",
        json={"test": "data", "message": "Hello Activemq"},
        headers={
    ...
    if response.status_code == 200:
        print(f"✓ Activemq webhook test passed (HTTP {response.status_code})")
        print(f"Response: {response.json()}")
    else:
        print(f"✗ Activemq webhook test failed (HTTP {response.status_code})")
        print(f"Response: {response.text}")
        sys.exit(1)
except Exception as e:
    print(f"✗ Activemq webhook test failed with error: {e}")
    sys.exit(1)

print("")
print("==========================================")
print("Activemq module tests passed! ✓")
print("==========================================")
EOF
