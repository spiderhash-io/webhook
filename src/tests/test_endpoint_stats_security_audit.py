...
class TestEndpointStatsInformationDisclosure:
    ...
    async def test_error_message_disclosure(self):
        ...
        try:
            ...
            for pattern in sensitive_patterns:
                assert pattern.lower() not in error_msg.lower(), f"Error message contains sensitive pattern: {pattern}"
    ...
    async def test_get_stats_information_disclosure(self):
        ...
        for pattern in sensitive_patterns:
            assert pattern.lower() not in stats_json.lower(), f"get_stats contains sensitive pattern: {pattern}"
        ...
        for input_val in problematic_inputs:
            ...
            try:
                ...
                if input_val in result or str(input_val) in result:
                    ...
                    assert False, f"Invalid input {input_val} was accepted without validation"
