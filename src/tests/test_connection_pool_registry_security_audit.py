...
class TestJSONParsingDoS:
    ...
    async def test_circular_reference_dos(self):
        ...
        try:
            ...
            assert "circular" in str(e).lower() or "recursion" in str(e).lower() or "maximum" in str(e).lower()
    ...
    async def test_deeply_nested_config_dos(self):
        ...
        try:
            ...
            assert "depth" in str(e).lower() or "recursion" in str(e).lower() or "maximum" in str(e).lower()
    ...
    async def test_non_serializable_config(self):
        ...
        try:
            pool = await registry.get_pool("conn1", non_serializable_config, mock_factory)
            assert pool is not None
...
class TestConfigHashCollision:
    ...
    async def test_hash_collision_detection(self):
        ...
        assert pool1 is not pool2, "Hash collision detected: different configs mapped to same pool"
...
class TestActiveRequestCounterManipulation:
    ...
    async def test_release_wrong_pool(self):
        ...
        pool_info1 = registry._pools.get("conn1")
        assert pool_info1 is not None
        assert pool_info1.active_requests == initial_requests  # Unchanged (not decremented)
