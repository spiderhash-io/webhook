...
def test_load_env_vars_with_default():
    ...
    config = {
        "key": "{$TEST_VAR_DEFAULT:default_value}"
    }
...
def test_load_env_vars_complex_nested():
    ...
    assert result["webhook"]["module-config"]["headers"]["Authorization"] == "Bearer secret_key_123"
...
def test_load_env_vars_multiple_in_same_string():
    ...
    config = {
        "url": "{$PROTOCOL}://{$DOMAIN}/{$PATH}/webhook"
    }
...
def test_load_env_vars_no_replacement_needed():
    ...
    config = {
        "normal_string": "no variables here",
        "number": 123,
        "boolean": True
    }
