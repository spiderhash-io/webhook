...
class TestWebhookEndpointTaskResultHandling:
    ...
    async def test_task_result_exception_disclosure(self):
        ...
        mock_task.result.side_effect = Exception("Internal error: password=secret, host=internal.db")
        
        mock_handler.process_webhook = AsyncMock(return_value=(
            {"data": "test"},
            {},
            mock_task
        ))
...
class TestWebhookEndpointErrorDisclosure:
    ...
    async def test_process_webhook_error_disclosure(self):
        ...
        # Create exception with sensitive information
        sensitive_error = Exception("Database error: connection string=postgresql://user:pass@host/db")
