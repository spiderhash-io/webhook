...
class TestOAuth2SSRF:
    ...
    async def test_introspection_endpoint_private_ip_ssrf(self):
        ...
        is_valid, message = await validator.validate(headers, b"")
        assert is_valid is False or "192.168" in message.lower() or "not allowed" in message.lower() or "private" in message.lower()
    ...
    async def test_introspection_endpoint_metadata_ssrf(self):
        ...
        is_valid, message = await validator.validate(headers, b"")
        assert is_valid is False or "169.254" in message.lower() or "metadata" in message.lower() or "not allowed" in message.lower()
    ...
    async def test_introspection_endpoint_file_protocol_ssrf(self):
        ...
        is_valid, message = await validator.validate(headers, b"")
        assert is_valid is False or "file://" in message.lower() or "not allowed" in message.lower()
...
class TestOAuth2JWTAlgorithmConfusion:
    ...
    async def test_jwt_empty_secret(self):
        ...
        assert is_valid is False
        assert "secret" in message.lower() or "not configured" in message.lower() or "Invalid" in message
...
class TestOAuth2ScopeEscalation:
    ...
    async def test_scope_injection_jwt(self):
        ...
        token = jwt.encode({
            "sub": "user123",
            "scope": "read write admin"  # Extra scopes
...
class TestOAuth2TimingAttacks:
    ...
    async def test_token_validation_timing(self):
        ...
        # Invalid tokens
        invalid_tokens = [
            jwt.encode({"sub": "user123"}, "wrong_secret", algorithm="HS256"),  # Wrong secret
...
class TestOAuth2IntrospectionSecurity:
    ...
    async def test_introspection_timeout_handling(self):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            mock_client.return_value.__aenter__.return_value.post = AsyncMock(side_effect=timeout_error)
            
            is_valid, message = await validator.validate(headers, b"")
            # Should fail due to timeout
            assert is_valid is False
            assert "timeout" in message.lower() or "network error" in message.lower() or "error" in message.lower()
...
class TestOAuth2EdgeCases:
    ...
    async def test_very_long_token(self):
        ...
        config = {
            "oauth2": {
                "validate_token": False  # Disable validation for this test
            }
        }
