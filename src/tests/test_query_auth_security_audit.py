...
class TestQueryAuthParameterPollution:
    ...
    async def test_parameter_name_similarity_attack(self):
        ...
        for similar_name in similar_names:
            ...
            assert is_valid is False
            assert "Missing required query parameter" in message or "Invalid API key" in message
...
class TestQueryAuthConfigurationSecurity:
    ...
    async def test_config_type_validation(self):
        ...
        invalid_configs = [
            ...
            {"query_auth": {"api_key": "secret", "case_sensitive": "true"}},  # Should be bool
        ]
    ...
    async def test_parameter_name_injection_via_config(self):
        ...
        malicious_configs = [
            ...
            {"query_auth": {"parameter_name": "api_key; DROP TABLE", "api_key": "secret"}},
            {"query_auth": {"parameter_name": "api_key\ninjected", "api_key": "secret"}},
        ]
...
class TestQueryAuthRegexRedos:
    ...
    async def test_regex_redos_parameter_name(self):
        ...
        is_valid, error = QueryParameterAuthValidator._validate_parameter_name(complex_name)
...
class TestQueryAuthEdgeCases:
    ...
    async def test_none_parameter_value(self):
        ...
        assert is_valid is False
        assert "missing" in message.lower() or "type" in message.lower() or "invalid" in message.lower()
