    ...
    async def test_startup_configmanager_error_disclosure(self):
        ...
        with patch('src.main.ConfigManager') as mock_config_manager_class:
            ...
            if "failed to initialize configmanager:" in output.lower():
                ...
                error_part = output.lower().split("failed to initialize configmanager:")[-1].strip()
    ...
    async def test_startup_clickhouse_error_disclosure(self):
        ...
        with patch('src.main.ConfigManager') as mock_config_manager_class:
            ...
            with patch('src.main.ClickHouseAnalytics') as mock_clickhouse_class:
                ...
                mock_clickhouse.connect = AsyncMock(side_effect=Exception("Connection failed: secret-password-123"))
        ...
        with patch('src.main.ConfigManager') as mock_config_manager_class:
            ...
            with patch.dict(os.environ, {"CONFIG_FILE_WATCHING_ENABLED": "true"}):
                ...
                with patch('src.main.ConfigFileWatcher') as mock_watcher_class:
                    mock_watcher = Mock()
                    mock_watcher.start = Mock(side_effect=Exception("/etc/webhooks.json: access denied"))
...
class TestCustomOpenAPIErrorDisclosure:
    ...
    def test_custom_openapi_error_disclosure(self):
        ...
        with patch('src.openapi_generator.generate_openapi_schema', side_effect=Exception("/etc/passwd: permission denied")):
            # Capture stdout
            stdout_capture = io.StringIO()
            with redirect_stdout(stdout_capture):
                # Call custom_openapi
            ...
            if "warning: failed to generate openapi schema:" in output.lower():
                ...
                error_part = output.lower().split("warning: failed to generate openapi schema:")[-1].strip()
                # The sanitized error should not contain sensitive paths
                assert "/etc/passwd" not in error_part, "Error message should be sanitized"
            else:
                # If format changed, at least verify sensitive path is not in the sanitized part
    ...
    def test_custom_openapi_config_manager_access(self):
        ...
        with patch('src.openapi_generator.generate_openapi_schema', return_value={"openapi": "3.0.0"}):
...
class TestShutdownErrorHandling:
    ...
    async def test_shutdown_error_handling(self):
        ...
        mock_config_manager.pool_registry.close_all_pools = AsyncMock(side_effect=Exception("Connection pool error: secret-password"))
...
class TestEnvironmentVariableInjection:
    ...
    def test_config_reload_debounce_seconds_injection(self):
        ...
        for invalid_value in invalid_values:
            with patch.dict(os.environ, {"CONFIG_RELOAD_DEBOUNCE_SECONDS": invalid_value}):
...
class TestStartupRaceConditions:
    ...
    async def test_startup_concurrent_initialization(self):
        ...
        with patch('src.main.ConfigManager') as mock_config_manager_class:
            ...
            assert success_count >= 0  # Can be 0 if there are race conditions, but shouldn't crash
...
class TestErrorMessageSanitization:
    ...
    def test_read_webhook_error_sanitization(self):
        ...
        with patch('src.main.WebhookHandler', side_effect=mock_handler_init):
            ...
            assert "/etc/passwd" not in error_detail, "Error message should be sanitized"
    ...
    def test_process_webhook_error_sanitization(self):
        ...
        with patch('src.main.WebhookHandler') as mock_handler_class:
            ...
            mock_handler.process_webhook = AsyncMock(side_effect=Exception("/etc/shadow: access denied"))
            mock_handler.config = {}
            ...
            # Should not contain sensitive paths
            assert "/etc/shadow" not in error_detail, "Error message should be sanitized"
