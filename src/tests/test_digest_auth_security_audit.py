...
def generate_digest_response(username: str, password: str, realm: str, method: str, uri: str, 
                             nonce: str, nc: str = "00000001", cnonce: str = "", qop: str = "auth") -> str:
...
class TestDigestAuthHeaderParsing:
    ...
    async def test_header_injection_via_newlines(self):
        ...
        malicious_header = 'Digest username="user", realm="Test Realm",\nX-Injected: header'
    ...
    async def test_regex_redos_via_complex_header(self):
        ...
        complex_header = 'Digest username="' + 'a' * 1000 + '", realm="' + 'b' * 1000 + '"'
...
class TestDigestAuthErrorDisclosure:
    ...
    async def test_exception_message_sanitization(self):
        ...
        with patch.object(validator, '_parse_digest_header', side_effect=Exception("Internal error with path: /etc/passwd")):
            ...
            assert "internal error" not in error_msg or "digest auth validation error" in error_msg
    ...
    async def test_config_exposure_in_errors(self):
        ...
        # Use wrong password
        response = generate_digest_response(
            "secret_user", "wrong_password", "Test Realm", "POST", uri, nonce, nc, cnonce
        )
