...
class TestRecaptchaErrorDisclosure:
    ...
    async def test_error_message_sanitization(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_client_instance = AsyncMock()
            mock_client.return_value.__aenter__.return_value = mock_client_instance
            mock_client_instance.post.side_effect = Exception("Connection error with secret: secret_key_12345")
            ...
            assert "secret" not in message.lower() or "secret key" in message.lower()  # May mention "secret key" but not the value
    ...
    async def test_httpx_error_sanitization(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_client_instance = AsyncMock()
            mock_client.return_value.__aenter__.return_value = mock_client_instance
            mock_client_instance.post.side_effect = httpx.HTTPError("Internal error with path: /etc/passwd")
            ...
            assert "internal error" not in error_msg or "failed to verify" in message.lower()
    ...
    async def test_generic_exception_sanitization(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_client_instance = AsyncMock()
            mock_client.return_value.__aenter__.return_value = mock_client_instance
            mock_client_instance.post.side_effect = Exception("Internal error with sensitive data")
            ...
            assert "recaptcha validation" in error_msg or "verification" in error_msg or "processing error" in error_msg
...
class TestRecaptchaDoS:
    ...
    async def test_timeout_handling(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            is_valid, message = await validator.validate(headers, body)
            assert is_valid is False
            assert "error" in message.lower() or "timeout" in message.lower() or "failed" in message.lower()
