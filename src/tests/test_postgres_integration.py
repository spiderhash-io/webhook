...
class TestPostgreSQLModuleConnectionPool:
    ...
    async def test_setup_with_valid_config(self):
        ...
        with patch.object(module, '_validate_hostname', return_value=True):
            ...
            with patch('src.modules.postgres.asyncpg.create_pool', side_effect=create_pool_mock):
                await module.setup()
    ...
    async def test_setup_with_connection_error(self):
        ...
        with patch('src.modules.postgres.asyncpg.create_pool', side_effect=Exception("Connection failed")):
    ...
    async def test_hybrid_mode_invalid_field_name_validation(self):
        ...
        config = {
            'module-config': {
                ...
                'schema': {
                    'fields': {
                        123: {'column': 'test', 'type': 'string'}  # Invalid: not a string
                    }
                }
            },
            'connection_details': {
        ...
        assert module.storage_mode == 'hybrid', f"Expected 'hybrid', got '{module.storage_mode}'"
        ...
        with patch.object(module, '_validate_hostname', return_value=True):
            ...
            assert "Field name must be a string" in error_msg or "got" in error_msg.lower() or "int" in error_msg.lower()
