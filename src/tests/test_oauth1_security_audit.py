...
class TestOAuth1PlaintextWeaknesses:
    ...
    async def test_plaintext_secret_exposure(self):
        ...
        assert "secret123" in signing_key or quote(consumer_secret, safe='') in signing_key
...
class TestOAuth1HeaderParsing:
    ...
    async def test_header_injection_via_newlines(self):
        ...
        malicious_header = 'OAuth oauth_consumer_key="consumer_key",\nX-Injected: header'
    ...
    async def test_regex_redos_via_complex_header(self):
        ...
        complex_header = 'OAuth oauth_consumer_key="' + 'a' * 1000 + '", oauth_signature_method="' + 'b' * 1000 + '"'
...
class TestOAuth1ErrorDisclosure:
    ...
    async def test_exception_message_sanitization(self):
        ...
        with patch.object(validator, '_parse_oauth_header', side_effect=Exception("Internal error with path: /etc/passwd")):
            ...
            assert "internal error" not in error_msg or "oauth 1.0 validation error" in error_msg
...
class TestOAuth1ConfigurationSecurity:
    ...
    async def test_config_type_validation(self):
        ...
        invalid_configs = [
            ...
            {"oauth1": {"consumer_key": "key", "consumer_secret": "secret", "signature_method": 123}},
        ]
...
class TestOAuth1URINormalization:
    ...
    async def test_uri_with_query_string(self):
        ...
        config = {
            ...
            "_request": type('obj', (object,), {
                'scope': {'path': '/webhook/test?param=value'}
            })()
        }
        validator = OAuth1Validator(config)
