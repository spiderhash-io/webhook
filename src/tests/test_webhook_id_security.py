...
class TestWebhookIDSecurity:
    ...
    def test_reserved_names_blocked(self):
        ...
        for name in reserved_names:
            ...
            assert not is_valid, f"Should reject reserved name (uppercase): {name.upper()}"
            
            # Test mixed case
            is_valid, msg = InputValidator.validate_webhook_id(name.capitalize())
            assert not is_valid, f"Should reject reserved name (mixed case): {name.capitalize()}"
    ...
    def test_reserved_prefixes_blocked(self):
        ...
        for prefix, expected_msg in reserved_prefixes:
            ...
            assert expected_msg in msg.lower() or "start with alphanumeric" in msg.lower()
    ...
    def test_extremely_long_ids_blocked(self):
        ...
        for long_id in long_ids:
            ...
            assert not is_valid, f"Should reject extremely long ID: {len(long_id)} chars"
            assert "too long" in msg.lower()
    ...
    def test_must_start_with_alphanumeric(self):
        ...
        for invalid_id in ['_webhook', '-webhook']:
            is_valid, msg = InputValidator.validate_webhook_id(invalid_id)
            assert not is_valid, f"Should reject ID starting with special char: {invalid_id}"
    ...
    def test_consecutive_special_chars_blocked(self):
        ...
        for invalid_id in invalid_ids:
            is_valid, msg = InputValidator.validate_webhook_id(invalid_id)
            assert not is_valid, f"Should reject ID with consecutive special chars: {invalid_id}"
    ...
    def test_only_special_chars_blocked(self):
        ...
        for invalid_id in invalid_ids:
            is_valid, msg = InputValidator.validate_webhook_id(invalid_id)
            assert not is_valid, f"Should reject ID with only special chars: {invalid_id}"
    ...
    def test_case_insensitive_reserved_names(self):
        ...
        for variation in variations:
            ...
            assert not is_valid, f"Should reject reserved name (case variation): {variation}"
