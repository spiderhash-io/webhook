...
MAX_ATTEMPTS_LIMIT = (
    20  # Maximum allowed retry attempts (prevents DoS via excessive retries)
)
...
class RetryHandler:
    ...
    def _validate_retry_config(
        ...
        if not isinstance(max_attempts, int):
            logger.warning(
                f"Invalid max_attempts type: {type(max_attempts)}, defaulting to 3"
            )
            max_attempts = 3
        if max_attempts < MIN_ATTEMPTS:
            logger.warning(
                f"max_attempts {max_attempts} is below minimum {MIN_ATTEMPTS}, setting to {MIN_ATTEMPTS}"
            )
            max_attempts = MIN_ATTEMPTS
        if max_attempts > MAX_ATTEMPTS_LIMIT:
            logger.warning(
                f"max_attempts {max_attempts} exceeds security limit {MAX_ATTEMPTS_LIMIT}, capping to {MAX_ATTEMPTS_LIMIT}"
            )
        ...
        if not isinstance(initial_delay, (int, float)):
            logger.warning(
                f"Invalid initial_delay type: {type(initial_delay)}, defaulting to 1.0"
            )
            initial_delay = 1.0
        if initial_delay < MIN_DELAY:
            logger.warning(
                f"initial_delay {initial_delay} is below minimum {MIN_DELAY}, setting to {MIN_DELAY}"
            )
            initial_delay = MIN_DELAY
        if initial_delay > MAX_DELAY_LIMIT:
            logger.warning(
                f"initial_delay {initial_delay} exceeds security limit {MAX_DELAY_LIMIT}, capping to {MAX_DELAY_LIMIT}"
            )
        ...
        if not isinstance(max_delay, (int, float)):
            logger.warning(
                f"Invalid max_delay type: {type(max_delay)}, defaulting to 60.0"
            )
            max_delay = 60.0
        if max_delay < MIN_DELAY:
            logger.warning(
                f"max_delay {max_delay} is below minimum {MIN_DELAY}, setting to {MIN_DELAY}"
            )
            max_delay = MIN_DELAY
        if max_delay > MAX_DELAY_LIMIT:
            logger.warning(
                f"max_delay {max_delay} exceeds security limit {MAX_DELAY_LIMIT}, capping to {MAX_DELAY_LIMIT}"
            )
        ...
        if max_delay < initial_delay:
            logger.warning(
                f"max_delay {max_delay} is less than initial_delay {initial_delay}, setting max_delay to {initial_delay}"
            )
        ...
        if not isinstance(backoff_multiplier, (int, float)):
            logger.warning(
                f"Invalid backoff_multiplier type: {type(backoff_multiplier)}, defaulting to 2.0"
            )
            backoff_multiplier = 2.0
        if backoff_multiplier < MIN_BACKOFF_MULTIPLIER:
            logger.warning(
                f"backoff_multiplier {backoff_multiplier} is below minimum {MIN_BACKOFF_MULTIPLIER}, setting to {MIN_BACKOFF_MULTIPLIER}"
            )
            backoff_multiplier = MIN_BACKOFF_MULTIPLIER
        if backoff_multiplier > MAX_BACKOFF_MULTIPLIER:
            logger.warning(
                f"backoff_multiplier {backoff_multiplier} exceeds security limit {MAX_BACKOFF_MULTIPLIER}, capping to {MAX_BACKOFF_MULTIPLIER}"
            )
        ...
        if not isinstance(retryable_errors, list):
            logger.warning(
                f"Invalid retryable_errors type: {type(retryable_errors)}, using defaults"
            )
            retryable_errors = DEFAULT_RETRYABLE_ERRORS
        else:
            # Filter to only valid string entries
            retryable_errors = [
                str(e) for e in retryable_errors if isinstance(e, str) and e
            ]

        non_retryable_errors = retry_config.get(
            "non_retryable_errors", DEFAULT_NON_RETRYABLE_ERRORS
        )
        if not isinstance(non_retryable_errors, list):
            logger.warning(
                f"Invalid non_retryable_errors type: {type(non_retryable_errors)}, using defaults"
            )
            non_retryable_errors = DEFAULT_NON_RETRYABLE_ERRORS
        else:
            # Filter to only valid string entries
            non_retryable_errors = [
                str(e) for e in non_retryable_errors if isinstance(e, str) and e
            ]
    ...
    def _is_retryable_error(
        ...
        full_error_name = (
            f"{error_module}.{error_type_name}" if error_module else error_type_name
        )
        ...
        logger.warning(
            f"Unknown error type '{full_error_name}' - defaulting to non-retryable for security"
        )
        return False
    ...
    def _calculate_backoff(
        ...
        try:
            ...
            if not (delay >= 0 and delay != float("inf") and not math.isnan(delay)):
                logger.warning(
                    f"Backoff calculation resulted in invalid value: {delay}, using max_delay"
                )
                delay = max_delay
        ...
        **kwargs,
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt > 0:
                    logger.info(
                        f"Module execution succeeded after {attempt + 1} attempts"
                    )
                return True, None
                ...
                if not self._is_retryable_error(
                    e, retryable_errors, non_retryable_errors
                ):
                ...
                if attempt < max_attempts - 1:
                    ...
                    logger.error(
                        f"Module execution failed after {max_attempts} attempts: {e}"
                    )
