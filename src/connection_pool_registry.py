...
class ConnectionPoolRegistry:
    ...
    def __init__(self, migration_timeout: float = 300.0):
        ...
        if not isinstance(migration_timeout, (int, float)):
            raise TypeError(
                f"migration_timeout must be a number, got {type(migration_timeout).__name__}"
            )
        ...
        if migration_timeout > 86400 * 365:  # Max 1 year (prevent overflow)
            raise ValueError(
                f"migration_timeout exceeds maximum limit (1 year), got {migration_timeout}"
            )

        self._pools: Dict[str, PoolInfo] = {}  # connection_name -> PoolInfo
        self._deprecated_pools: Dict[str, PoolInfo] = (
            {}
        )  # connection_name -> PoolInfo (old version)
    ...
    async def get_pool(
        ...
        if not isinstance(connection_name, str):
            raise TypeError(
                f"connection_name must be a string, got {type(connection_name).__name__}"
            )
        if not connection_name:
            raise ValueError("connection_name cannot be empty")
        if len(connection_name) > 256:  # Prevent DoS via extremely long names
            raise ValueError(
                f"connection_name too long: {len(connection_name)} characters (max: 256)"
            )
        ...
        if not isinstance(connection_config, dict):
            raise TypeError(
                f"connection_config must be a dictionary, got {type(connection_config).__name__}"
            )

        # Security: Validate pool_factory is callable
        if not callable(pool_factory):
            raise TypeError(
                f"pool_factory must be callable, got {type(pool_factory).__name__}"
            )
        ...
        async with self._get_lock():
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to create connection pool: {error_msg}"
                ) from e
            ...
            for connection_name in to_remove:
                ...
                try:
                    ...
                    error_msg = sanitize_error_message(
                        e, "ConnectionPoolRegistry.cleanup_deprecated_pools"
                    )
                    print(
                        f"Warning: Error closing deprecated pool {connection_name}: {error_msg}"
                    )
        ...
        if connection_name in self._pools:
            ...
            return {
                ...
                "created_at": datetime.fromtimestamp(
                    pool_info.created_at, tz=timezone.utc
                ).isoformat(),
            ...
            return {
                ...
                "deprecated_at": (
                    datetime.fromtimestamp(
                        pool_info.deprecated_at, tz=timezone.utc
                    ).isoformat()
                    if pool_info.deprecated_at
                    else None
                ),
    ...
    def get_all_pools_info(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        result = {}
        for connection_name in set(self._pools.keys()) | set(
            self._deprecated_pools.keys()
        ):
    ...
    async def close_all_pools(self) -> None:
        ...
        async with self._get_lock():
            all_pools = list(self._pools.values()) + list(
                self._deprecated_pools.values()
            )
            for pool_info in all_pools:
                try:
                    ...
                    error_msg = sanitize_error_message(
                        e, "ConnectionPoolRegistry.close_all_pools"
                    )
                    print(
                        f"Warning: Error closing pool {pool_info.connection_name}: {error_msg}"
                    )
