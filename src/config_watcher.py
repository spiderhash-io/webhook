    ...
    def _debounce_reload(self, file_path: str):
        ...
        with self._lock:
            ...
            self._debounce_timer = threading.Timer(
                self.debounce_seconds, self._trigger_reload, args=(file_path,)
            )
    ...
    def _trigger_reload(self, file_path: str):
        ...
        if not loop:
            try:
                ...
                threading.Thread(
                    target=self._run_reload_in_thread, args=(file_path,), daemon=True
                ).start()
                return

        # Schedule reload task in event loop
        if loop and loop.is_running():
            asyncio.run_coroutine_threadsafe(self._async_reload(file_path), loop)
        else:
    ...
    async def _async_reload(self, file_path: str):
        ...
        try:
            ...
            sanitized_error = sanitize_error_message(
                e, "ConfigFileWatcher._async_reload"
            )
...
class ConfigFileWatcher:
    ...
    def start(self):
        ...
        watch_dir = (
            os.path.dirname(webhook_path) or os.path.dirname(connection_path) or "."
        )
        ...
        self.handler = ConfigFileHandler(
            self.config_manager, self.debounce_seconds, self.event_loop
        )
