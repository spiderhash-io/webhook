...
_default_webhooks = (
    "config/development/webhooks.json"
    if os.path.exists("config/development/webhooks.json")
    else "webhooks.json"
)
_default_connections = (
    "config/development/connections.json"
    if os.path.exists("config/development/connections.json")
    else "connections.json"
)
...
def _validate_connection_host(host: str, connection_type: str) -> str:
    ...
    if len(host) > MAX_HOST_LENGTH:
        raise ValueError(
            f"{connection_type} host too long: {len(host)} characters (max: {MAX_HOST_LENGTH})"
        )
    ...
    for char in dangerous_chars:
        if char in host:
            raise ValueError(
                f"{connection_type} host contains dangerous character '{char}': '{host}'"
            )
    ...
    try:
        ...
        allow_private = (
            os.getenv("ALLOW_PRIVATE_IP_CONNECTIONS", "false").lower() == "true"
        )
...
def _validate_connection_port(port: Any, connection_type: str) -> int:
    ...
    if port_int < 1 or port_int > 65535:
        raise ValueError(
            f"{connection_type} port must be between 1 and 65535: {port_int}"
        )
...
async def inject_connection_details(
    webhook_config_data: Dict[str, Any], connection_config: Dict[str, Any]
) -> Dict[str, Any]:
    """
        ...
        if connection_name:
            ...
            if connection_details:
                ...
                if not connection_type:
                    raise ValueError(
                        f"Connection '{connection_name}' is missing required 'type' field"
                    )
