...
╚███╔███╔╝███████╗██████╔╝██║  ██║╚██████╔╝╚██████╔╝██║  ██╗
 ╚══╝╚══╝ ╚══════╝╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ╚═╝  ╚═╝
    ...
    try:
        ...
        app.state.webhook_config_data = await inject_connection_details(
            webhook_config_data, connection_config
        )
    ...
    if clickhouse_config:
        ...
        try:
            ...
            print(
                "✅ ClickHouse event logger initialized - all webhook events will be logged"
            )
        except Exception as e:
            # SECURITY: Sanitize error message to prevent information disclosure
    ...
    webhook_connect_enabled = (
        os.getenv("WEBHOOK_CONNECT_ENABLED", "false").lower() == "true"
    )
        ...
        try:
            ...
            try:
                ...
                if debounce_seconds < 0.1:
                    ...
                    print(
                        f"WARNING: CONFIG_RELOAD_DEBOUNCE_SECONDS too small, using default 3.0"
                    )
                elif debounce_seconds > 3600:
                    debounce_seconds = 3600  # Cap at 1 hour
                    print(
                        f"WARNING: CONFIG_RELOAD_DEBOUNCE_SECONDS too large, capped at 3600"
                    )
            except (ValueError, TypeError):
                # Invalid value, use default
                debounce_seconds = 3.0
                print(
                    f"WARNING: Invalid CONFIG_RELOAD_DEBOUNCE_SECONDS value '{debounce_str}', using default 3.0"
                )

            app.state.config_watcher = ConfigFileWatcher(
                app.state.config_manager, debounce_seconds=debounce_seconds
            )
            app.state.config_watcher.start()
            print(
                f"✅ Config file watcher started - automatic reload enabled (debounce: {debounce_seconds}s)"
            )
        except Exception as e:
            # SECURITY: Sanitize error message to prevent information disclosure
            sanitized_error = sanitize_error_message(
                e, "startup_logic.ConfigFileWatcher"
            )
...
async def shutdown_logic(app: FastAPI):
    ...
    if hasattr(app.state, "config_watcher") and app.state.config_watcher:
        try:
            ...
            sanitized_error = sanitize_error_message(
                e, "shutdown_logic.ConfigFileWatcher"
            )
    ...
    if (
        hasattr(app.state, "webhook_connect_channel_manager")
        and app.state.webhook_connect_channel_manager
    ):
        try:
...
openapi_url_path = (
    ...
    if (ROOT_PATH and not DISABLE_OPENAPI_DOCS)
    else ("/openapi.json" if not DISABLE_OPENAPI_DOCS else None)
)
app = FastAPI(
    lifespan=lifespan,
    ...
    root_path=(
        ROOT_PATH if ROOT_PATH else None
    ),  # Set root_path for reverse proxy support (URL generation only)
)
...
if cors_origins_env:
    ...
    for origin in raw_origins:
        ...
        if origin == "*" or origin == "null":
            print(
                f"WARNING: CORS origin '{origin}' is rejected for security. Use specific origins only."
            )
            continue
        ...
        if not (origin.startswith("http://") or origin.startswith("https://")):
            print(
                f"WARNING: CORS origin '{origin}' must start with http:// or https://. Skipping."
            )
            continue
        ...
        if (
            ...
            if domain_part != domain_only:
                print(
                    f"WARNING: CORS origin '{origin}' contains path/fragment/query/userinfo. Use only domain: '{origin.split('://')[0]}://{domain_only}'. Skipping."
                )
                continue
        ...
        if origin.startswith("http://localhost") or origin.startswith(
            ...
            print(
                f"WARNING: CORS origin '{origin}' uses localhost. Consider using 127.0.0.1 or specific domain."
            )
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: StarletteRequest, call_next):
        ...
        if is_https:
            ...
            try:
                hsts_max_age = int(
                    os.getenv("HSTS_MAX_AGE", "31536000")
                )  # Default: 1 year
...
async def default_endpoint(request: Request):
    ...
    is_allowed, remaining = await rate_limiter.check_rate_limit(
        default_key, max_requests=default_rate_limit, window_seconds=60
    )
...
async def health_endpoint(request: Request):
    ...
    webhook_connect_manager = getattr(
        request.app.state, "webhook_connect_channel_manager", None
    )
    if webhook_connect_manager:
        try:
            # Check if channel manager is running and buffer is connected
            if (
                hasattr(webhook_connect_manager, "is_running")
                and webhook_connect_manager.is_running()
            ):
                components["webhook_connect"] = "healthy"
            else:
                components["webhook_connect"] = (
                    "degraded"  # Initialized but buffer not connected
                )
        except Exception:
...
async def stats_endpoint(request: Request):
    ...
    if stats_auth_token:
        ...
        if not hmac.compare_digest(
            token.encode("utf-8"), stats_auth_token.encode("utf-8")
        ):
    ...
    is_allowed, remaining = await rate_limiter.check_rate_limit(
        stats_key, max_requests=stats_rate_limit, window_seconds=60
    )
...
async def reload_config_endpoint(request: Request):
    ...
    try:
        ...
        reload_webhooks = (
            bool(reload_webhooks_raw) if reload_webhooks_raw is not None else True
        )
        reload_connections = (
            bool(reload_connections_raw) if reload_connections_raw is not None else True
        )
        validate_only = (
            bool(validate_only_raw) if validate_only_raw is not None else False
        )
    except Exception:
        # Default: reload both
    ...
    if reload_webhooks and reload_connections:
        ...
        return JSONResponse(
            content={"status": "error", "error": "No reload operation specified"}
        )
...
async def config_status_endpoint(request: Request):
    ...
    status["file_watching_enabled"] = (
        config_watcher is not None and config_watcher.is_watching()
    )
