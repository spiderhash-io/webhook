...
class GCPPubSubModule(BaseModule):
    ...
    def __init__(self, config: Dict[str, Any], **kwargs):
        ...
        if not GCP_PUBSUB_AVAILABLE:
            raise ImportError(
                "google-cloud-pubsub library is required for GCP Pub/Sub module. Install with: pip install google-cloud-pubsub"
            )
    ...
    def _validate_topic_name(self, topic_name: str) -> str:
        ...
        if len(topic_name) > MAX_TOPIC_LENGTH:
            raise ValueError(
                f"Topic name too long: {len(topic_name)} characters (max: {MAX_TOPIC_LENGTH})"
            )
        ...
        try:
            ...
            if credentials_path:
                ...
                if ".." in credentials_path or ".." in decoded_path:
                    raise ValueError(
                        "Invalid credentials path (path traversal detected)"
                    )

                # Block absolute paths
                if credentials_path.startswith("/") or decoded_path.startswith("/"):
                    raise ValueError(
                        "Invalid credentials path (absolute path not allowed)"
                    )
                ...
                if len(credentials_path) >= 2 and credentials_path[1] == ":":
                    raise ValueError(
                        "Invalid credentials path (Windows absolute path not allowed)"
                    )
    ...
    async def process(self, payload: Any, headers: Dict[str, str]) -> None:
        ...
        try:
            ...
            if isinstance(payload, (dict, list)):
                try:
                    ...
                    message_data = json.dumps(
                        {
                            "error": "Payload too deeply nested to serialize",
            ...
            topic_path = self.publisher.topic_path(
                self.project_id, self._validated_topic
            )
            ...
            try:
                ...
                try:
                    print(
                        f"Topic '{self._validated_topic}' not found, attempting to create it..."
                    )
                    ...
                    print(
                        f"Created topic '{self._validated_topic}' in project '{self.project_id}'"
                    )
                    ...
                    print(
                        f"Published message to GCP Pub/Sub topic '{self._validated_topic}'"
                    )
                except Exception as create_error:
                    # If creation fails, raise original NotFound error
                    sanitized_error = sanitize_error_message(
                        create_error, "GCP Pub/Sub topic creation"
                    )
                    print(f"ERROR [GCP Pub/Sub topic creation]: {sanitized_error}")
                    raise Exception(
                        f"Topic '{self._validated_topic}' not found in project '{self.project_id}' and could not be created: {sanitized_error}"
                    )
        except Exception as e:
