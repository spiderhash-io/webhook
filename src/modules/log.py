...
class LogModule(BaseModule):
    ...
    def _sanitize_for_logging(
        self, data: Any, visited: Set[int] = None, depth: int = 0
    ) -> str:
        """
        ...
        try:
                ...
                for key, value in data.items():
                    ...
                    if should_redact:
                        ...
                        if is_sensitive:
                            ...
                            sanitized_dict[key] = self._sanitize_for_logging(
            ...
            result = re.sub(
                r"[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F]", "[CTRL]", result
        ...
        for key, value in headers.items():
            if should_redact:
                ...
                for sensitive in self.SENSITIVE_KEYS:
                    ...
                    if (
                        sensitive.replace("_", "-") in key_lower
                        or sensitive in key_lower
                    ):
                        is_sensitive = True
                        break
    ...
    async def process(self, payload: Any, headers: Dict[str, str]) -> None:
        ...
        module_config = self.config.get("module-config", {})
        pretty_print = (
            module_config.get("pretty_print", False)
                ...
                for key, value in data.items():
                    if should_redact:
                        ...
                        if is_sensitive:
                            ...
                            sanitized_dict[key] = self._create_sanitized_dict(
                                value, visited, depth + 1
                            )
                        else:
