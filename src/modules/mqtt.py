...
class MQTTModule(BaseModule):
    ...
    def _validate_topic_name(self, topic_name: str) -> str:
        ...
        if len(topic_name.encode("utf-8")) > MAX_TOPIC_LENGTH:
            raise ValueError(
                f"Topic name too long: {len(topic_name.encode('utf-8'))} bytes (max: {MAX_TOPIC_LENGTH})"
            )
        ...
        if "\x00" in topic_name:
            raise ValueError(
                "Topic name cannot contain null bytes (forbidden control character)"
            )
        ...
        if "--" in topic_name:
            raise ValueError(
                "Topic name contains dangerous pattern: '--' (not allowed)"
            )
        if ".." in topic_name:
            raise ValueError(
                "Topic name contains dangerous pattern: '..' (not allowed)"
        ...
        for pattern in dangerous_patterns:
            if pattern in topic_name:
                raise ValueError(
                    f"Topic name contains dangerous pattern: '{pattern}' (not allowed)"
                )

        # Block wildcards (security: don't allow publishing to wildcard topics)
        if "+" in topic_name or "#" in topic_name:
            raise ValueError(
                "Topic name cannot contain wildcards (+ or #) for published topics"
            )
        ...
        if topic_name.startswith("$"):
            raise ValueError(
                "Topic name cannot start with '$' (reserved for system topics)"
            )
    ...
    async def process(self, payload: Any, headers: Dict[str, str]) -> None:
        ...
        try:
            ...
            # Handle Sonoff/Tasmota format
                ...
                tasmota_type = self.module_config.get(
                    "tasmota_type", "cmnd"
                )  # cmnd, stat, or tele
            ...
            if topic_prefix:
                ...
                if ".." in topic_prefix:
                    raise ValueError(
                        "Topic prefix contains dangerous pattern: '..' (path traversal not allowed)"
                    )
                if "--" in topic_prefix:
                    raise ValueError(
                        "Topic prefix contains dangerous pattern: '--' (not allowed)"
                    )
                if "+" in topic_prefix or "#" in topic_prefix:
                    raise ValueError("Topic prefix cannot contain wildcards (+ or #)")
                if topic_prefix.startswith("$"):
                    raise ValueError(
                        "Topic prefix cannot start with '$' (reserved for system topics)"
                    )
                if "//" in topic_prefix:
            ...
            await self.client.publish(
                topic=topic, payload=message, qos=qos, retain=retained
            )

            print(
                f"Message published to MQTT topic: {topic} (QoS: {qos}, Retained: {retained})"
            )
