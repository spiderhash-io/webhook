...
class MySQLModule(BaseModule):
    ...
    def _validate_table_name(self, table_name: str) -> str:
        ...
        if len(table_name) > 64:
            raise ValueError(
                f"Table name too long: {len(table_name)} characters (max: 64)"
            )
    ...
    def _validate_column_name(self, column_name: str) -> str:
        ...
        if len(column_name) > 64:
            raise ValueError(
                f"Column name too long: {len(column_name)} characters (max: 64)"
            )
    ...
    def _validate_upsert_key(self, upsert_key: str) -> str:
        ...
        if len(upsert_key) > 64:
            raise ValueError(
                f"Upsert key too long: {len(upsert_key)} characters (max: 64)"
            )
    ...
    def _validate_index_name(self, index_name: str) -> str:
        ...
        if len(index_name) > 64:
            raise ValueError(
                f"Index name too long: {len(index_name)} characters (max: 64)"
            )
        ...
        if self.pool_registry and connection_name and self.connection_details:
            ...
            try:
                self.pool = await self.pool_registry.get_pool(
                    connection_name, self.connection_details, create_mysql_pool
                )
                # Test connection
                ...
                print(
                    f"Warning: Failed to get pool from registry, falling back to direct creation: {e}"
                )
        ...
        try:
            ...
            if self.storage_mode == "json":
                ...
                for field_name, field_config in self.schema["fields"].items():
                    ...
                    column_type = self._get_mysql_type(
            ...
            if self.schema and "indexes" in self.schema:
                for index_name, index_config in self.schema["indexes"].items():
                    ...
                    if index_columns:
                        quoted_columns = ", ".join(
                            [self._quote_identifier(col) for col in index_columns]
                        )
                ...
                if self.upsert and self.upsert_key:
                    ...
                    async with self.pool.acquire() as conn:
                        async with conn.cursor() as cur:
                            await cur.execute(
                                query,
                                (webhook_id, timestamp, payload_json, headers_json),
                            )
                            await conn.commit()
                else:
                    # Regular insert
