...
class SaveToDiskModule(BaseModule):
    ...
    def _validate_path(self, path: str, base_dir: str = None) -> str:
        ...
        if os.path.exists(real_path) and not os.path.isdir(real_path):
            raise ValueError(
                f"Path points to an existing file, not a directory: {path}"
            )
    ...
    async def process(self, payload: Any, headers: Dict[str, str]) -> None:
        ...
        if base_dir is not None:
            ...
            try:
                ...
                is_temp_dir = any(
                    base_dir_lower.startswith(prefix)
                ...
                if not is_temp_dir:
                    ...
                    all_blocked = (
                        system_dirs + var_blocked_subdirs + private_var_blocked_subdirs
                    )
                    ...
                    for sys_dir in all_blocked:
                        ...
                        # Check resolved path
                        if (
                            base_dir_lower == sys_dir_lower
                            or base_dir_lower.startswith(sys_dir_lower + "/")
                        ):
                            raise ValueError(
                                f"base_dir cannot be a system directory: {base_dir_real}"
                            )
                        # Check original absolute path (before symlink resolution)
                        if (
                            base_dir_abs_lower == sys_dir_lower
                            or base_dir_abs_lower.startswith(sys_dir_lower + "/")
                        ):
                            raise ValueError(
                                f"base_dir cannot be a system directory: {base_dir_abs}"
                            )
