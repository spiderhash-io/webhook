...
class ActiveMQModule(BaseModule):
    ...
    def __init__(self, config: Dict[str, Any], **kwargs):
        ...
        if not ACTIVEMQ_AVAILABLE:
            raise ImportError(
                "stomp.py library is required for ActiveMQ module. Install with: pip install stomp.py"
            )
        ...
        self.destination_type = self.module_config.get(
    ...
    def _validate_destination(self, destination: str) -> str:
        ...
        if len(destination) > MAX_DESTINATION_LENGTH:
            raise ValueError(
                f"Destination too long: {len(destination)} characters (max: {MAX_DESTINATION_LENGTH})"
            )
        ...
        for prefix in reserved_prefixes:
            if destination_lower.startswith(prefix.lower()):
                raise ValueError(
                    f"Destination cannot start with reserved prefix: '{prefix}'"
                )
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            if host.lower() in dangerous_hostnames:
                raise ValueError(
                    f"Host '{host}' is blocked for security (metadata service)"
                )
            ...
            try:
                ...
                if ip.is_link_local:
                    raise ValueError(
                        f"Host '{host}' is blocked for security (link-local address)"
                    )

                # Block loopback
                if ip.is_loopback:
                    raise ValueError(
                        f"Host '{host}' is blocked for security (loopback address)"
                    )

                # Block multicast
                if ip.is_multicast:
                    raise ValueError(
                        f"Host '{host}' is blocked for security (multicast address)"
                    )

                # Block reserved addresses
                if ip.is_reserved:
                    raise ValueError(
                        f"Host '{host}' is blocked for security (reserved address)"
                    )
