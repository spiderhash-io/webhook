...
class RedisPublishModule(BaseModule):
    ...
    def _validate_channel_name(self, channel_name: str) -> str:
        ...
        if len(channel_name) > 255:
            raise ValueError(
                f"Channel name too long: {len(channel_name)} characters (max: 255)"
            )
        ...
        for pattern in dangerous_patterns:
            if pattern in channel_name:
                raise ValueError(
                    f"Channel name contains dangerous pattern: '{pattern}'"
                )
        ...
        for keyword in redis_keywords:
            ...
            if (
                channel_name_lower == keyword
                or channel_name_lower.startswith(keyword + ".")
                or channel_name_lower.startswith(keyword + "_")
            ):
                raise ValueError(
                    f"Channel name contains forbidden Redis keyword: '{keyword}'"
                )
    ...
    async def process(self, payload: Any, headers: Dict[str, str]) -> None:
        ...
        client = redis.Redis(
            host=host, port=port, socket_connect_timeout=5, socket_timeout=5
        )
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to connect to Redis at {host}:{port}: {sanitize_error_message(e, 'Redis connection')}"
            )
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to publish to Redis channel '{channel}': {sanitize_error_message(e, 'Redis publish')}"
            )
