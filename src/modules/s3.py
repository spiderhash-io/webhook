...
class S3Module(BaseModule):
    ...
    def __init__(self, config: Dict[str, Any], **kwargs):
        ...
        raw_filename_pattern = self.module_config.get(
            "filename_pattern", "webhook_{uuid}.json"
        )
        self._validated_prefix = self._validate_s3_path_component(raw_prefix, "prefix")
        self._validated_filename_pattern = self._validate_filename_pattern(
            raw_filename_pattern
        )

    def _validate_s3_path_component(
        self, path_component: str, component_name: str
    ) -> str:
        """
        ...
        if len(path_component) > 255:
            raise ValueError(
                f"{component_name} too long: {len(path_component)} characters (max: 255)"
            )
        ...
        if (
            ...
            or path_component.endswith("/")
        ):
            raise ValueError(
                f"{component_name} contains path traversal or invalid characters: '{path_component}'"
            )
        ...
        for pattern in dangerous_patterns:
            if pattern in path_component:
                raise ValueError(
                    f"{component_name} contains dangerous pattern: '{pattern}'"
                )
    ...
    def _validate_filename_pattern(self, filename_pattern: str) -> str:
        ...
        if len(filename_pattern) > 255:
            raise ValueError(
                f"Filename pattern too long: {len(filename_pattern)} characters (max: 255)"
            )

        # Reject path traversal
        if (
            ".." in filename_pattern
            or "/" in filename_pattern
            or "\\" in filename_pattern
        ):
            raise ValueError(
                f"Filename pattern contains path traversal or invalid characters: '{filename_pattern}'"
            )
        ...
        for pattern in dangerous_patterns:
            if pattern in filename_pattern:
                raise ValueError(
                    f"Filename pattern contains dangerous pattern: '{pattern}'"
                )
    ...
    def _validate_object_key(self, object_key: str) -> str:
        ...
        if len(object_key.encode("utf-8")) > 1024:
            raise ValueError(
                f"Object key too long: {len(object_key.encode('utf-8'))} bytes (max: 1024)"
            )
    ...
    async def process(self, payload: Any, headers: Dict[str, str]) -> None:
        ...
        if not re.match(r"^[a-zA-Z0-9_\-\.]+$", filename):
            raise ValueError(
                f"Generated filename contains invalid characters: '{filename}'"
            )
