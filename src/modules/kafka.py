...
class KafkaModule(BaseModule):
    ...
    def _validate_topic_name(self, topic_name: str) -> str:
        ...
        if len(topic_name) > 249:
            raise ValueError(
                f"Topic name too long: {len(topic_name)} characters (max: 249)"
            )
        ...
        for keyword in kafka_keywords:
            ...
            if (
                topic_name_lower == keyword
                or topic_name_lower.startswith(keyword + ".")
                or topic_name_lower.startswith(keyword + "_")
                or topic_name_lower.startswith(keyword + "-")
            ):
                raise ValueError(
                    f"Topic name contains forbidden Kafka keyword: '{keyword}'"
                )
    ...
    async def setup(self) -> None:
        ...
        bootstrap_servers = self.connection_details.get(
