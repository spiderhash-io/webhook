...
def generate_webhook_path(
    webhook_id: str, config: Dict[str, Any]
            ...
            "parameters": [
                {
                    ...
                    "schema": {"type": "string", "example": sanitized_webhook_id},
                }
            ],
                ...
                if isinstance(required_scopes, list):
                    for scope in required_scopes:
                        ...
                        flows["clientCredentials"]["scopes"][
                            sanitized_scope
                        ] = f"Required scope: {sanitized_scope}"
            else:
...
def extract_request_body(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if data_type == "json":
        content["application/json"] = {"schema": extract_request_schema(config)}
    elif data_type == "blob":
        ...
        content["application/json"] = {
            "schema": {"type": "object", "description": "JSON payload"}
        }
...
def extract_request_schema(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if data_type == "json":
        ...
        return {"type": "string", "format": "binary", "description": "Binary data"}
...
def extract_security_info(config: Dict[str, Any]) -> Dict[str, str]:
    ...
    if config.get("hmac"):
        ...
        info["HMAC Verification"] = (
            f"{sanitized_algorithm} signature in {sanitized_header} header"
        )
            ...
            "content": {
                "application/json": {
                    "schema": {
                        "type": "object",
                        "properties": {
                            "message": {"type": "string", "example": "200 OK"}
                        },
                    }
                }
            },
            ...
            "content": {
                "application/json": {
                    "schema": {
                        "type": "object",
                        "properties": {
                            "detail": {"type": "string", "example": "Access denied"}
                        },
                    }
                }
            },
