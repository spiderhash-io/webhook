...
class InputValidator:
    ...
    def validate_payload_size(payload: bytes) -> Tuple[bool, str]:
        ...
        if len(payload) > InputValidator.MAX_PAYLOAD_SIZE:
            return (
                False,
                f"Payload too large: {len(payload)} bytes (max: {InputValidator.MAX_PAYLOAD_SIZE})",
            )
    ...
    def validate_headers(headers: Dict[str, str]) -> Tuple[bool, str]:
        ...
        if len(headers) > InputValidator.MAX_HEADER_COUNT:
            return (
                False,
                f"Too many headers: {len(headers)} (max: {InputValidator.MAX_HEADER_COUNT})",
            )
        ...
        dangerous_chars = [
            ...
            "\u2028",
            "\u2029",
        ]  # Include Unicode line/paragraph separators
            ...
            if isinstance(header_value, str):
                for char in dangerous_chars:
                    if char in header_value:
                        return (
                            False,
                            f"Invalid header value: contains forbidden character",
                        )

        # Calculate total header size (only count string values to avoid errors)
        total_size = sum(
            len(k) + (len(v) if isinstance(v, str) else 0) for k, v in headers.items()
        )
        if total_size > InputValidator.MAX_HEADER_SIZE:
            return (
                False,
                f"Headers too large: {total_size} bytes (max: {InputValidator.MAX_HEADER_SIZE})",
            )
    ...
    def validate_json_depth(
        ...
        try:
            if current_depth > InputValidator.MAX_JSON_DEPTH:
                return (
                    False,
                    f"JSON too deeply nested: {current_depth} levels (max: {InputValidator.MAX_JSON_DEPTH})",
                )

            if isinstance(obj, dict):
                for value in obj.values():
                    is_valid, msg = InputValidator.validate_json_depth(
                        value, current_depth + 1, visited
                    )
                    if not is_valid:
                        return is_valid, msg
        ...
        try:
            if isinstance(obj, str):
                if len(obj) > InputValidator.MAX_STRING_LENGTH:
                    return (
                        False,
                        f"String too long: {len(obj)} chars (max: {InputValidator.MAX_STRING_LENGTH})",
                    )
            elif isinstance(obj, dict):
                for value in obj.values():
                    is_valid, msg = InputValidator.validate_string_length(
                        value, visited
                    )
                    if not is_valid:
                        return is_valid, msg
    ...
    def validate_webhook_id(webhook_id: str) -> Tuple[bool, str]:
        ...
        if len(webhook_id) > MAX_WEBHOOK_ID_LENGTH:
            return (
                False,
                f"Webhook ID too long: {len(webhook_id)} characters (max: {MAX_WEBHOOK_ID_LENGTH})",
            )
        ...
        if not re.match(r"^[a-zA-Z0-9][a-zA-Z0-9_-]*$", webhook_id):
            return (
                False,
                "Invalid webhook ID format. Must start with alphanumeric and contain only alphanumeric, underscore, and hyphen characters",
            )
    ...
    @staticmethod
    def validate_all(
        webhook_id: str, payload_bytes: bytes, headers: Dict[str, str], payload_obj: Any
    ) -> Tuple[bool, str]:
        """Run all validations."""
        # Validate webhook ID
