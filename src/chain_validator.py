...
class ChainValidator:
    ...
    MAX_CHAIN_LENGTH = (
        20  # Maximum allowed chain length (prevents DoS via excessive chains)
    )
    ...
    def validate_chain_config(config: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if chain_length < ChainValidator.MIN_CHAIN_LENGTH:
            return (
                False,
                f"Chain must contain at least {ChainValidator.MIN_CHAIN_LENGTH} module",
            )

        if chain_length > ChainValidator.MAX_CHAIN_LENGTH:
            return (
                False,
                f"Chain length {chain_length} exceeds security limit {ChainValidator.MAX_CHAIN_LENGTH}",
            )
        ...
        if "chain-config" in config:
            ...
            if chain_config is not None:
                is_valid, error = ChainValidator._validate_chain_execution_config(
                    chain_config
                )
                if not is_valid:
                    return False, error
    ...
    def _validate_chain_item(item: Any, index: int) -> Tuple[bool, Optional[str]]:
        ...
        if isinstance(item, str):
            ...
            if not module_name or not isinstance(module_name, str):
                return (
                    False,
                    f"Chain item {index}: module name must be a non-empty string",
                )
            ...
            try:
                ...
                return (
                    False,
                    f"Chain item {index}: module '{module_name}' is not registered",
                )
            ...
            if not module_name.strip():
                return (
                    False,
                    f"Chain item {index}: module name cannot be empty or whitespace-only",
                )
            ...
            if connection is not None and not isinstance(connection, str):
                return (
                    False,
                    f"Chain item {index}: 'connection' must be a string if provided",
                )
            ...
            if module_config is not None and not isinstance(module_config, dict):
                return (
                    False,
                    f"Chain item {index}: 'module-config' must be a dictionary if provided",
                )
            ...
            for deprecated_key in deprecated_top_level:
                if deprecated_key in item:
                    ...
                    if module_config and deprecated_key in module_config:
                        return (
                            False,
                            f"Chain item {index}: '{deprecated_key}' should be in 'module-config', not at top level",
                        )
            ...
            if retry_config is not None:
                if not isinstance(retry_config, dict):
                    return (
                        False,
                        f"Chain item {index}: 'retry' must be a dictionary if provided",
                    )

                # Validate retry config structure (basic validation, detailed validation in retry_handler)
                if "enabled" in retry_config and not isinstance(
                    retry_config["enabled"], bool
                ):
                    return (
                        False,
                        f"Chain item {index}: 'retry.enabled' must be a boolean",
                    )
                ...
                if "max_attempts" in retry_config:
                    ...
                    if not isinstance(max_attempts, int) or max_attempts < 1:
                        return (
                            False,
                            f"Chain item {index}: 'retry.max_attempts' must be a positive integer",
                        )
            ...
            for field in item.keys():
                if field not in allowed_fields:
                    return (
                        False,
                        f"Chain item {index}: unknown field '{field}' (allowed: {', '.join(sorted(allowed_fields))})",
                    )
            ...
            return (
                False,
                f"Chain item {index}: must be a string or dictionary, got {type(item).__name__}",
            )

    @staticmethod
    def _validate_chain_execution_config(
        ...
        if execution not in allowed_execution_modes:
            return (
                False,
                f"chain-config.execution must be one of {allowed_execution_modes}, got '{execution}'",
            )
        ...
        for field in chain_config.keys():
            if field not in allowed_fields:
                return (
                    False,
                    f"chain-config: unknown field '{field}' (allowed: {', '.join(sorted(allowed_fields))})",
                )
