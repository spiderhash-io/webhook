...
# Go to the project root (script is now in scripts/)
...
for i in {1..30}; do
    if docker-compose -f docker/compose/docker-compose.yaml exec -T clickhouse wget --spider -q http://localhost:8123/ping 2>/dev/null; then
        echo "ClickHouse is ready!"
        break
    fi
    if [ $i -eq 30 ]; then
        echo "ERROR: ClickHouse did not become ready in time"
        docker-compose -f docker/compose/docker-compose.yaml logs clickhouse
        exit 1
    fi
    sleep 2
done
...
echo
echo "To view logs:"
echo "  docker-compose -f docker/compose/docker-compose.yaml logs -f webhook-1"
echo
echo "To check ClickHouse data:"
echo "  docker-compose -f docker/compose/docker-compose.yaml exec clickhouse clickhouse-client --query 'SELECT count() FROM webhook_logs'"
echo
echo "To stop services:"
echo "  docker-compose -f docker/compose/docker-compose.yaml down"
echo
