...
class TestHMACTimingAttacks:
    ...
    async def test_timing_attack_resistance_detailed(self):
        ...
        # Correct signature
        correct_sig = hmac.new(b"test_secret_key_12345", body, hashlib.sha256).hexdigest()
        ...
        incorrect_signatures = [
            ...
            correct_sig[:10] + 'a' * (len(correct_sig) - 10),  # Middle wrong (use 'a' not 'x' for valid hex)
        ]
        ...
        start = time.time()
        is_valid_correct, _ = await validator.validate({"x-hmac-signature": correct_sig}, body)
        ...
        assert max_time_diff < 0.1, f"Timing difference too large: {max_time_diff}s (potential timing attack)"
...
class TestHMACAlgorithmConfusion:
    ...
    async def test_algorithm_mismatch_sha1_vs_sha256(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sha1_sig}, body)
        assert is_valid is False
    ...
    async def test_algorithm_mismatch_sha256_vs_sha512(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sha256_sig}, body)
        assert is_valid is False
        assert "Invalid HMAC signature" in message
...
class TestHMACHeaderManipulation:
    ...
    async def test_case_insensitive_header_name(self):
        ...
        is_valid, message = await validator_upper.validate({"x-hmac-signature": sig}, body)
...
class TestHMACBodyManipulation:
    ...
    async def test_body_tampering_detected(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sig}, modified_body)
        assert is_valid is False
        assert "Invalid HMAC signature" in message
    ...
    async def test_very_large_body(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sig}, large_body)
        assert is_valid is True
    ...
    async def test_unicode_body_handling(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sig}, unicode_body)
        assert is_valid is True
    ...
    async def test_null_bytes_in_body(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sig}, body_with_nulls)
...
class TestHMACEdgeCases:
    ...
    async def test_unicode_in_signature(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": unicode_sig}, body)
        ...
        assert is_valid is False
        assert "Invalid HMAC signature format" in message or "Invalid HMAC signature" in message
    ...
    async def test_signature_truncation_attack(self):
        ...
        for trunc_sig in truncated:
            is_valid, message = await validator.validate({"x-hmac-signature": trunc_sig}, body)
            assert is_valid is False
    ...
    async def test_signature_padding_attack(self):
        ...
        for pad_sig in padded:
            is_valid, message = await validator.validate({"x-hmac-signature": pad_sig}, body)
            assert is_valid is False
...
class TestHMACReplayAttacks:
    ...
    async def test_signature_with_timestamp(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": sig}, old_body)
        assert is_valid is False
...
class TestHMACSecretExposure:
    ...
    async def test_secret_not_in_error_messages(self):
        ...
        is_valid, message = await validator.validate({"x-hmac-signature": "wrong"}, body)
        assert is_valid is False
