...
class TestJWTAlgorithmConfusion:
    ...
    async def test_none_algorithm_in_token_header(self):
        ...
        config = {"jwt": {"secret": secret, "algorithm": "HS256"}}
        ...
        try:
            ...
            assert (
                "Invalid JWT algorithm" in message
                or "Invalid JWT token format" in message
            )
        except Exception:
    ...
    async def test_algorithm_header_manipulation(self):
        ...
        secret = "jwt_secret_key"
        config = {"jwt": {"secret": secret, "algorithm": "HS256"}}  # Config says HS256
        ...
        if len(parts) == 3:
            ...
            assert is_valid is False
            assert (
                "Invalid JWT signature" in message or "Invalid JWT algorithm" in message
            )
...
class TestJWTKeyConfusion:
    ...
    async def test_empty_secret_rejected(self):
        ...
        config = {"jwt": {"secret": "", "algorithm": "HS256"}}  # Empty secret
    ...
    async def test_weak_secret_detection(self):
        ...
        for weak_secret in weak_secrets:
            config = {"jwt": {"secret": weak_secret, "algorithm": "HS256"}}
...
class TestJWTHeaderInjection:
    ...
    async def test_malformed_header_json(self):
        ...
        malformed_header = (
            base64.urlsafe_b64encode(b"{invalid json}").decode().rstrip("=")
        )
        payload = (
            base64.urlsafe_b64encode(json.dumps({"user": "test"}).encode())
            .decode()
            .rstrip("=")
        )
        signature = "invalid"
        ...
        assert is_valid is False
        assert (
            "Invalid JWT token format" in message or "JWT validation failed" in message
        )
...
class TestJWTClaimConfusion:
    ...
    async def test_exp_claim_bypass_attempts(self):
        ...
        secret = "jwt_secret_key"
        config = {"jwt": {"secret": secret, "algorithm": "HS256", "verify_exp": True}}
    ...
    async def test_issuer_case_sensitivity(self):
        ...
        secret = "jwt_secret_key"
        config = {
            "jwt": {"secret": secret, "algorithm": "HS256", "issuer": "ExpectedIssuer"}
        }
...
class TestJWTStructureAttacks:
    ...
    async def test_empty_signature(self):
        ...
        assert is_valid is False
        assert (
            "Invalid JWT signature" in message or "Invalid JWT token format" in message
        )
    ...
    async def test_invalid_base64_encoding(self):
        ...
        for token in invalid_tokens:
            ...
            assert is_valid is False
            assert (
                "Invalid JWT token format" in message
                or "JWT validation failed" in message
            )
...
class TestJWTSecretHandling:
    ...
    async def test_whitespace_only_secret(self):
        ...
        config = {"jwt": {"secret": "   ", "algorithm": "HS256"}}  # Only whitespace
        ...
        assert (
            ...
            or "cannot be empty" in message.lower()
        )
