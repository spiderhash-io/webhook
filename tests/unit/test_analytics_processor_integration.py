...
class TestAnalyticsProcessorConnection:
    ...
    async def test_connect_with_valid_config(self):
        ...
        with patch(
            "src.analytics_processor._validate_connection_host",
            return_value="localhost",
        ), patch("asyncio.get_running_loop") as mock_loop, patch(
            "src.analytics_processor.Client", return_value=mock_client
        ), patch(
            "src.analytics_processor.ClickHouseAnalytics"
        ) as mock_ch_class:
...
class TestAnalyticsProcessorGetAllWebhookIds:
    ...
    async def test_get_all_webhook_ids_success(self):
        ...
        mock_client = MagicMock()
        mock_client.execute = Mock(
            return_value=[("webhook1",), ("webhook2",), ("webhook3",)]
        )
...
class TestAnalyticsProcessorProcessAndSaveStats:
    ...
    async def test_process_and_save_stats_success(self):
        ...
        with patch.object(
            processor,
            "get_all_webhook_ids",
            AsyncMock(return_value=["webhook1", "webhook2"]),
        ), patch.object(
            processor,
            "calculate_stats",
            AsyncMock(
                side_effect=[{"total": 10, "minute": 1}, {"total": 20, "minute": 2}]
            ),
        ):
    ...
    async def test_process_and_save_stats_with_error(self):
        ...
        with patch.object(
            processor, "get_all_webhook_ids", AsyncMock(side_effect=Exception("Error"))
        ):
...
class TestAnalyticsProcessingLoop:
    ...
    async def test_analytics_processing_loop_with_config(self):
        ...
        with patch("src.analytics_processor.load_env_vars", return_value=config), patch(
            "src.analytics_processor.AnalyticsProcessor"
