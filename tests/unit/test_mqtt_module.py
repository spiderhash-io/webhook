...
class TestMQTTTopicValidation:
    ...
    def test_consecutive_slashes_rejected(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            MQTTModule(config)
        assert (
            "consecutive" in str(exc_info.value).lower()
            or "slashes" in str(exc_info.value).lower()
        )
...
class TestMQTTConfiguration:
    ...
    def test_invalid_qos_level(self):
        ...
        config = {
            ...
            "topic": "test/topic",
            "connection_details": {"host": "localhost", "port": 1883},
            "module-config": {"qos": 3},  # Invalid QoS
        }
        module = MQTTModule(config)
    ...
    def test_topic_prefix(self):
        ...
        config = {
            ...
            "topic": "events",
            "connection_details": {"host": "localhost", "port": 1883},
            "module-config": {"topic_prefix": "webhook"},
        }
        module = MQTTModule(config)
...
class TestShellyCompatibility:
    ...
    def test_shelly_gen2_format(self):
        ...
        config = {
            ...
            "connection_details": {"host": "localhost", "port": 1883},
            "module-config": {"shelly_gen2_format": True, "device_id": "device123"},
        }
        module = MQTTModule(config)
