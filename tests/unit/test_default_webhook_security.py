...
class TestDefaultWebhookSecurityConcerns:
    ...
    async def test_default_webhook_shows_all_data_for_debugging(self):
        ...
        with patch('builtins.print') as mock_print:
            await log_module.process(sensitive_payload, {"content-type": "application/json"})
            
            # Get all printed output
            all_output = " ".join(str(call) for call in mock_print.call_args_list)
            
            # All data should be visible (no redaction for debugging)
            assert "secret123" in all_output, "Password should be visible for debugging!"
            assert "key_abc123" in all_output, "API key should be visible for debugging!"
            assert "jwt_token_here" in all_output, "Token should be visible for debugging!"
    ...
    async def test_default_headers_are_visible_for_debugging(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert "secret_token_12345" in all_output, "Authorization header should be visible!"
    ...
    async def test_sensitive_headers_are_redacted_when_enabled(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert "secret_token_12345" not in all_output, "Authorization header not redacted!"
    ...
    async def test_redaction_defaults_to_enabled_when_not_specified(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert "secret123" not in all_output, "Should redact by default for security!"
            assert "[REDACTED]" in all_output
    ...
    async def test_default_webhook_has_no_authorization(self):
        ...
        assert "authorization" not in default_config["default"], \
    ...
    async def test_default_webhook_fallback_logs_warning(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert warning_logged, "No warning logged when falling back to default webhook!"
    ...
    async def test_newlines_in_payload_are_sanitized(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert "INJECTED_LOG_LINE" not in all_output.split("\n")[1:], \
    ...
    async def test_control_characters_are_sanitized(self):
        ...
        """
        log_module = LogModule({
            "module-config": {"pretty_print": False}
        })
        
        injection_payload = {
            "data": "normal\x00null\x08backspace\x1bescape"
        }
    ...
    async def test_default_webhook_logs_startup_warning(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            assert any("webhooks.json not found" in call for call in warning_calls), \
                "No warning about missing webhooks.json!"
            assert any("default logging" in call.lower() for call in warning_calls), \
    ...
    async def test_default_webhook_config_is_correct_structure(self):
        ...
        assert "authorization" not in config["default"], \
            "Default should not have auth (but this should be documented)"
        assert "rate_limit" not in config["default"], \
