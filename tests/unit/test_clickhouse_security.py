...
class TestClickHouseSecurity:
    ...
    def test_valid_table_names(self):
        ...
        for table_name in valid_names:
            config = {"module": "clickhouse", "module-config": {"table": table_name}}
    ...
    def test_sql_injection_attempts(self):
        ...
        for malicious_name in injection_attempts:
            config = {
                "module": "clickhouse",
                "module-config": {"table": malicious_name},
            }
            with pytest.raises(ValueError) as exc_info:
                ClickHouseModule(config)
            assert (
                "Invalid table name" in str(exc_info.value)
                or "forbidden keyword" in str(exc_info.value)
                or "dangerous pattern" in str(exc_info.value)
            )
    ...
    def test_sql_keywords_rejected(self):
        ...
        for keyword in sql_keywords:
            config = {
                "module": "clickhouse",
                "module-config": {"table": f"webhook_{keyword}_logs"},
            }
            with pytest.raises(ValueError) as exc_info:
                ClickHouseModule(config)
    ...
    def test_dangerous_patterns_rejected(self):
        ...
        for pattern in dangerous_patterns:
            config = {"module": "clickhouse", "module-config": {"table": pattern}}
            with pytest.raises(ValueError) as exc_info:
                ClickHouseModule(config)
            assert "dangerous pattern" in str(
                exc_info.value
            ).lower() or "Invalid table name" in str(exc_info.value)
    ...
    def test_special_characters_rejected(self):
        ...
        for name in special_chars:
            config = {"module": "clickhouse", "module-config": {"table": name}}
            with pytest.raises(ValueError) as exc_info:
                ClickHouseModule(config)
    ...
    def test_table_name_length_limit(self):
        ...
        config = {"module": "clickhouse", "module-config": {"table": long_name}}
        with pytest.raises(ValueError) as exc_info:
            ClickHouseModule(config)
    ...
    def test_table_name_with_numbers(self):
        ...
        config = {
            "module": "clickhouse",
            "module-config": {"table": "webhook_logs_2024_01"},
        }
        module = ClickHouseModule(config)
    ...
    def test_table_name_underscore_only(self):
        ...
        config = {
            "module": "clickhouse",
            "module-config": {"table": "webhook__logs"},  # Double underscore
        }
        module = ClickHouseModule(config)
    ...
    def test_table_name_starts_with_number(self):
        ...
        config = {
            "module": "clickhouse",
            "module-config": {"table": "2024_webhook_logs"},
        }
        module = ClickHouseModule(config)
    ...
    def test_multiple_injection_attempts(self):
        ...
        for pattern, description in injection_patterns:
            config = {
                "module": "clickhouse",
                "module-config": {"table": f"webhook_logs{pattern}"},
            }
            with pytest.raises(ValueError) as exc_info:
                ClickHouseModule(config)
