...
class TestQueryParameterInjection:
    ...
    def test_parameter_value_sanitization(self):
        ...
        for value in valid_values:
            sanitized, is_valid = QueryParameterAuthValidator._sanitize_parameter_value(
                value
            )
            assert is_valid, f"Valid value '{value}' was rejected"
            assert (
                sanitized == value
            ), f"Valid value '{value}' was modified: {sanitized}"
        ...
        for value, expected, reason in control_char_values:
            ...
            assert (
                sanitized == expected
            ), f"Value with {reason} was not sanitized correctly: '{sanitized}' != '{expected}'"
    ...
    async def test_query_params_with_injection_attempts(self, config):
        ...
        for param_name, param_value, reason in injection_attempts:
            ...
            if (
                ...
                assert (
                    not is_valid
                ), f"Injection attempt ({reason}) should fail: sanitized value doesn't match expected key"
            else:
                ...
                assert (
                    is_valid
                ), f"Sanitized value should match expected key after removing control characters ({reason})"
    ...
    async def test_query_params_invalid_parameter_name_in_config(self):
        ...
        invalid_configs = [
            ...
            {
                "query_auth": {"parameter_name": "api;key", "api_key": "secret"}
            },  # Semicolon
            {
                "query_auth": {"parameter_name": "api\x00key", "api_key": "secret"}
            },  # Null byte
            {
        ...
        for config in invalid_configs:
            ...
            assert (
                not is_valid
            ), f"Invalid parameter name in config was accepted: {config['query_auth']['parameter_name']}"
            assert (
