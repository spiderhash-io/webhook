...
class TestErrorInformationDisclosure:
    ...
    async def test_exception_error_disclosure_in_reload_webhooks(self):
        ...
        try:
            ...
            with patch(
                "builtins.open", side_effect=Exception("/etc/passwd: permission denied")
            ):
    ...
    async def test_json_decode_error_disclosure(self):
        ...
        try:
            ...
            assert (
                "/tmp" not in error_msg or len([c for c in error_msg if c == "/"]) < 3
            )
    ...
    def test_config_file_watcher_error_disclosure(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(
                config_manager, "reload_webhooks", new_callable=AsyncMock
            ) as mock_reload:
...
class TestFilePathValidation:
    ...
    async def test_double_encoded_path_traversal(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch(
                "builtins.open",
                side_effect=Exception(f"{double_encoded}: access denied"),
            ):
                result = await config_manager.reload_webhooks()
...
class TestErrorSanitization:
    ...
    async def test_reload_connections_uses_sanitize_error_message(self):
        ...
        try:
            ...
            # Mock open to raise exception with sensitive info
            with patch(
                "builtins.open", side_effect=Exception("/etc/shadow: access denied")
            ):
                result = await config_manager.reload_connections()
    ...
    def test_config_file_watcher_uses_sanitize_error_message(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(
                config_manager, "reload_webhooks", side_effect=mock_reload
            ):
                ...
                if "error during config reload:" in output.lower():
                    ...
                    error_part = (
                        output.lower().split("error during config reload:")[-1].strip()
                    )
                    ...
                    sanitized_lines = [
                        l
                        for l in lines
                        if "processing error" in l or "error occurred" in l
                    ]
                    if sanitized_lines:
                        assert (
