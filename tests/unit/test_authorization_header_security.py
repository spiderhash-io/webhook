...
class TestAuthorizationHeaderSecurity:
    ...
    async def test_timing_attack_resistance(self):
        ...
        time_diff_ratio_early = abs(median_valid - median_invalid_early) / max(
            median_valid, median_invalid_early, 0.000001
        )
        time_diff_ratio_late = abs(median_valid - median_invalid_late) / max(
            median_valid, median_invalid_late, 0.000001
        )
        time_diff_ratio_length = abs(median_valid - median_invalid_length) / max(
            median_valid, median_invalid_length, 0.000001
        )
    ...
    async def test_token_extraction_with_spaces(self):
        ...
        config = {"authorization": "Bearer token_with_spaces"}
        validator = AuthorizationValidator(config)
