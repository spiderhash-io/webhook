...
class TestMySQLModule:
    ...
    def test_validate_table_name_too_long(self):
        ...
        config = {"connection_details": {}, "module-config": {"table": "a" * 65}}
    ...
    async def test_process_json_mode(self, basic_config, mock_pool):
        ...
        assert (
            mock_pool.acquire.return_value.__aenter__.return_value.cursor.return_value.__aenter__.return_value.execute.called
        )
    ...
    @pytest.mark.asyncio
    async def test_ensure_table_creates_indexes_after_table(
        ...
        execute_calls = []
        original_execute = (
            mock_pool.acquire.return_value.__aenter__.return_value.cursor.return_value.__aenter__.return_value.execute
        )
        ...
        if len(index_queries) > 0:
            ...
            first_table_pos = next(
                i for i, q in enumerate(execute_calls) if "CREATE TABLE" in q.upper()
            )
            first_index_pos = next(
                i for i, q in enumerate(execute_calls) if "CREATE INDEX" in q.upper()
            )
            assert (
                first_table_pos < first_index_pos
