...
class TestLiveReloadIntegration:
    ...
    def temp_webhook_config(self, tmp_path):
        ...
        config = {
            "test_webhook_1": {"data_type": "json", "module": "log"},
            "test_webhook_2": {"data_type": "json", "module": "log"},
        }
    ...
    @pytest.fixture
    def config_manager(
        self, temp_webhook_config, temp_connection_config, pool_registry
    ):
    ...
    @pytest.mark.asyncio
    async def test_pool_migration_on_connection_change(
        self, config_manager, pool_registry, temp_connection_config
    ):
        ...
        pool1 = await pool_registry.get_pool(
            "conn1", connection_config1, mock_pool_factory
        )
        ...
        assert (
            connection_config2 is not None
        ), "Connection config 'conn1' should exist after reload"
        pool2 = await pool_registry.get_pool(
            "conn1", connection_config2, mock_pool_factory
        )
    ...
    @pytest.mark.asyncio
    async def test_webhook_addition_without_restart(
        ...
        new_config = {
            "test_webhook_1": {"data_type": "json", "module": "log"},
            "test_webhook_2": {"data_type": "json", "module": "log"},
            "test_webhook_3": {"data_type": "json", "module": "log"},
        }
