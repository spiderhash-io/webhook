...
class TestSaveToDiskSecurityAudit:
    ...
    async def test_configuration_injection_base_dir_path_traversal(self, temp_base_dir):
        ...
        for malicious_base in malicious_base_dirs:
            config = {
                "module": "save_to_disk",
                "module-config": {"base_dir": malicious_base, "path": "webhooks"},
            }
            ...
            assert any(
                ...
                for keyword in ["error", "validation", "system directory", "traversal"]
    ...
    async def test_dos_excessive_file_creation(self, temp_base_dir):
        ...
        config = {
            "module": "save_to_disk",
            "module-config": {"base_dir": temp_base_dir, "path": "webhooks"},
        }
        module = SaveToDiskModule(config)
    ...
    async def test_race_condition_directory_creation(self, temp_base_dir):
        ...
        config = {
            "module": "save_to_disk",
            "module-config": {"base_dir": temp_base_dir, "path": "race_test/subdir"},
        }
        module = SaveToDiskModule(config)
    ...
    async def test_windows_path_issues(self, temp_base_dir):
        ...
        for path in windows_paths:
            ...
            try:
                ...
                assert (
                    temp_base_dir in validated
                    or os.path.commonpath([temp_base_dir, validated]) == temp_base_dir
                )
            except Exception as e:
    ...
    async def test_configuration_validation_base_dir_type(self):
        ...
        config = {
            "module": "save_to_disk",
            "module-config": {"base_dir": 123, "path": "webhooks"},  # Invalid type
        }
    ...
    async def test_configuration_validation_path_type(self, temp_base_dir):
        ...
        config = {
            "module": "save_to_disk",
            "module-config": {"base_dir": temp_base_dir, "path": 123},  # Invalid type
        }
    ...
    async def test_path_length_limits(self, temp_base_dir):
        ...
        try:
            ...
            assert (
                ...
                or "name too long" in str(e).lower()
            )
    ...
    async def test_concurrent_symlink_creation_race(self, temp_base_dir):
        ...
        config = {
            "module": "save_to_disk",
            "module-config": {"base_dir": temp_base_dir, "path": "race_symlink"},
        }
        module = SaveToDiskModule(config)
    ...
    async def test_file_permission_race_condition(self, temp_base_dir):
        ...
        if os.path.exists(webhook_dir):
            ...
            for filename in files:
                ...
                assert (
                    file_mode == 0o600
                ), f"File {filename} has incorrect permissions: {oct(file_mode)}"
    ...
    async def test_double_encoded_path_traversal(self, temp_base_dir):
        ...
        for path in double_encoded_paths:
            ...
            if ".." in decoded:
                ...
                assert any(
                    keyword in error_msg
                    for keyword in ["error", "validation", "traversal"]
    ...
    async def test_empty_path_handling(self, temp_base_dir):
        ...
        config = {
            "module": "save_to_disk",
            "module-config": {"base_dir": temp_base_dir, "path": ""},  # Empty path
        }
        module = SaveToDiskModule(config)
