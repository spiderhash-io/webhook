...
class TestRetryHandlerSecurity:
    ...
    async def test_negative_max_attempts(self):
        ...
        success, error = await handler.execute_with_retry(
            failing_function, retry_config=malicious_config
        )
    ...
    async def test_zero_max_attempts(self):
        ...
        assert (
            call_count >= 1
        ), "Should execute at least once even with zero max_attempts"
    ...
    async def test_zero_delays(self):
        ...
        start_time = time.time()
        success, error = await handler.execute_with_retry(
            failing_function, retry_config=config
        )
        elapsed = time.time() - start_time
    ...
    async def test_error_classification_bypass_string_matching(self):
        ...
        success, error = await handler.execute_with_retry(
            function_with_bypass_error, retry_config=retry_config
        )
    ...
    async def test_default_retryable_unknown_errors(self):
        ...
        success, error = await handler.execute_with_retry(
            function_with_unknown_error, retry_config=retry_config
        )
        ...
        assert (
            call_count == 1
        ), f"Unknown errors should not be retried (fail-safe), but was retried {call_count} times"
    ...
    async def test_concurrent_retry_exhaustion(self):
        ...
        for i in range(50):
            task = handler.execute_with_retry(
                lambda: failing_function(i), retry_config=retry_config
            )
            tasks.append(task)
        ...
        # All should fail
        assert all(
            not r[0] if isinstance(r, tuple) else False for r in results
        ), "All should fail"
    ...
    async def test_backoff_calculation_edge_cases(self):
        ...
        for attempt, initial, max_delay, multiplier in test_cases:
            ...
            assert delay != float(
                "inf"
            ), f"Backoff delay should not be infinite: {delay}"
            assert (
                delay == delay
            ), f"Backoff delay should not be NaN: {delay}"  # NaN != NaN
            assert (
                delay <= max_delay or max_delay == 0.0
            ), f"Backoff should cap at max_delay: {delay} > {max_delay}"
