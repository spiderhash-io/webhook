...
class TestRedisPublishErrorDisclosure:
    ...
    async def test_error_message_sanitization(self):
        ...
        with patch("redis.Redis") as mock_redis_class:
            ...
            mock_client.ping.side_effect = redis.ConnectionError(
                "Connection failed: password=secret_password, host=8.8.8.8"
            )
            mock_redis_class.return_value = mock_client
            ...
            try:
                ...
                assert (
                    "redis connection" in error_msg or "processing error" in error_msg
                )
    ...
    async def test_redis_details_not_exposed(self):
        ...
        with patch("redis.Redis") as mock_redis_class:
            mock_client = MagicMock()
            mock_client.ping.return_value = True
            mock_client.publish.side_effect = redis.ConnectionError(
                "Redis internal error: AUTH failed"
            )
            mock_redis_class.return_value = mock_client
...
class TestRedisPublishConfigurationSecurity:
    ...
    async def test_config_type_validation(self):
        ...
        invalid_configs = [
            {
                "module": "redis_publish",
                "redis": {"host": "8.8.8.8", "port": 6379, "channel": None},
            },
            {
                "module": "redis_publish",
                "redis": {"host": "8.8.8.8", "port": 6379, "channel": 123},
            },
            {
                "module": "redis_publish",
                "redis": {"host": "8.8.8.8", "port": 6379, "channel": []},
            },
            {
                "module": "redis_publish",
                "redis": {"host": "8.8.8.8", "port": 6379, "channel": {}},
        ...
        for invalid_config in invalid_configs:
            try:
                ...
                assert module._validated_channel is None or isinstance(
                    module._validated_channel, str
                )
...
class TestRedisPublishMessageSerialization:
    ...
    async def test_message_structure(self):
        ...
        with patch("redis.Redis") as mock_redis_class:
            ...
            message = (
                call_args[0][1] if call_args[0] and len(call_args[0]) > 1 else None
            )
...
class TestRedisPublishChannelNameValidation:
    ...
    def test_channel_name_regex_redos(self):
        ...
        try:
            config = {
                "module": "redis_publish",
                "redis": {"host": "8.8.8.8", "port": 6379, "channel": complex_name},
            }
            RedisPublishModule(config)
            assert False, "Should have raised ValueError"
        except ValueError:
        ...
        for host in hex_hosts:
            ...
            try:
                ...
                assert (
                    ...
                    or "loopback" in str(e).lower()
                    or "not allowed" in str(e).lower()
                    or "invalid hostname" in str(e).lower()
                )
...
class TestRedisPublishAllowedHosts:
    ...
    def test_allowed_hosts_empty_list(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            RedisPublishModule(config)
        assert "not in the allowed hosts whitelist" in str(
            exc_info.value
        ) or "whitelist is empty" in str(exc_info.value)
