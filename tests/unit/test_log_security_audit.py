...
class TestLogInjection:
    ...
    async def test_newline_injection_in_headers(self):
        ...
        assert (
            "[NL]" in output or "[CTRL]" in output
        ), "Newlines in data should be sanitized"
    ...
    async def test_carriage_return_injection_in_payload(self):
        ...
        malicious_payload = {"data": "normal\r[WARNING] System compromised"}
        ...
        assert (
            "[NL]" in output or "[CTRL]" in output
        ), "Control characters should be replaced with safe markers"
    ...
    async def test_control_character_injection(self):
        ...
        malicious_payload = {"data": "normal\x00\x01\x02[FAKE] Log entry"}
...
class TestConfigSensitiveData:
    ...
    async def test_connection_details_exposure(self):
        ...
        assert (
            "postgresql://user:pass@host/db" not in output
        ), "Database URL should be redacted"
        assert (
            "[REDACTED - connection details]" in output
        ), "Connection details should be redacted"
