    ...
    def test_authorization_header_case_insensitive(self):
        ...
        with patch.dict(os.environ, {"CONFIG_RELOAD_ADMIN_TOKEN": "secret-token-123"}):
            ...
            for headers in headers_variants:
                ...
                assert response.status_code != 401 or "Authentication required" not in str(response.json())
...
class TestConfigFilePathTraversal:
    ...
    def test_path_traversal_in_webhook_config_file(self):
        ...
        for traversal_path in traversal_paths:
            ...
            assert not os.path.exists(traversal_path) or os.path.abspath(traversal_path) != os.path.abspath("../../etc/passwd")
...
class TestReloadDoS:
    ...
    async def test_rapid_reload_requests(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            json.dump({"conn": {"type": "rabbitmq", "host": "example.com", "port": 5672}}, f)
            temp_conn = f.name
        ...
        try:
            ...
            success_count = sum(1 for r in results if isinstance(r, ReloadResult) and r.success)
            error_count = sum(1 for r in results if isinstance(r, ReloadResult) and not r.success)
    ...
    async def test_connection_config_error_disclosure(self):
        ...
        try:
            ...
            if not result.success:
                ...
                assert "password" not in error_msg or "password" in "missing password"  # Only generic messages
        ...
        try:
            ...
            for i in range(10):
                ...
                with open(temp_conn, 'w') as f:
                    json.dump({f"conn{i}": {"type": "rabbitmq", "host": f"example{i}.com", "port": 5672}}, f)
