...
class TestConnectionTimeoutHandling:
    ...
    async def test_read_timeout_handled(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook", "timeout": 1.0},
        }
        module = HTTPWebhookModule(config)
...
class TestConnectionErrorTypes:
    ...
    async def test_dns_resolution_failure(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://nonexistent-domain-12345.com/webhook"},
        }
        module = HTTPWebhookModule(config)
    ...
    async def test_connection_refused(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com:9999/webhook"},
        }
        module = HTTPWebhookModule(config)
    ...
    async def test_network_unreachable(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook"},  # Use public domain
        }
        module = HTTPWebhookModule(config)
...
class TestQueryParameterForwarding:
    ...
    async def test_query_parameters_forwarded(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook?webhook_id=test123"},
        }
        module = HTTPWebhookModule(config)
...
class TestHTTPMethodPreservation:
    ...
    async def test_post_method_used(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook", "method": "POST"},
        }
        module = HTTPWebhookModule(config)
    ...
    async def test_patch_method_used(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook", "method": "PATCH"},
        }
        module = HTTPWebhookModule(config)
...
class TestConcurrentForwardingRequests:
    ...
    async def test_concurrent_requests_handled(self):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            tasks = [module.process({"test": f"data_{i}"}, {}) for i in range(10)]
...
class TestRuntimeURLValidation:
    ...
    def test_url_validated_at_init(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://localhost/webhook"},  # Should be blocked
        }
...
class TestIdempotencyKeyForwarding:
    ...
    async def test_idempotency_header_forwarded(self):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            headers = {"Idempotency-Key": "test-key-123", "X-Request-ID": "req-456"}
            ...
            assert (
                "Idempotency-Key" in forwarded_headers
                or "idempotency-key" in forwarded_headers
            )
