...
class TestSQLInjectionTableColumnNames:
    ...
    def test_index_name_sql_injection_attempts(self):
        ...
        assert hasattr(module, "_validate_index_name") or hasattr(
    ...
    def test_upsert_key_dangerous_characters(self):
        ...
        for dangerous_key in dangerous_keys:
            ...
            try:
                ...
                assert hasattr(
                    module, "_validate_upsert_key"
                ), "upsert_key validation missing"
            except ValueError:
...
class TestSSRFPrevention:
    ...
    async def test_ssrf_private_ip_blocked(self):
        ...
        for ip in private_ips:
            ...
            with patch(
                "src.modules.mysql.aiomysql.create_pool",
                side_effect=mock_create_pool_func,
            ):
                # Should not raise ValueError for private IPs (they're allowed now)
    ...
    async def test_connection_error_sanitization(self):
        ...
        with patch(
            ...
            side_effect=Exception(
                "Access denied for user 'test_user'@'db.example.com' (using password: YES)"
            ),
        ):
            ...
            assert (
                "password" not in error_msg.lower()
                or "using password" not in error_msg.lower()
            )
