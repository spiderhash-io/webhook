...
class TestRequestBodyReadingDoS:
    ...
    async def test_body_reading_exception_handling(self):
        ...
        try:
            ...
            assert handler._cached_body is None or isinstance(handler._cached_body, Exception)
        except Exception as e:
...
class TestRequestBodyCachingRaceConditions:
    ...
    async def test_concurrent_body_reading(self):
        ...
        results = await asyncio.gather(*[validate() for _ in range(10)], return_exceptions=True)
        
        # Body should only be read once (cached after first read)
        # However, without locking, there could be a race condition
        assert body_read_count <= 10, f"Race condition: body read {body_read_count} times instead of once"
    ...
    async def test_concurrent_validate_and_process(self):
        ...
        assert body_read_count == 1, f"Race condition: body read {body_read_count} times instead of once"
        assert handler._cached_body == original_body
    ...
    async def test_body_returns_non_bytes(self):
        ...
        for body_value, expected_type in test_cases:
            ...
            try:
                ...
                assert False, f"Unexpected error type for {expected_type.__name__}: {type(e).__name__}: {e}"
...
class TestRequestBodyErrorHandling:
    ...
    async def test_body_reading_information_disclosure(self):
        ...
        sensitive_error = Exception("Failed to read body from /etc/passwd: permission denied")
        ...
        try:
            ...
            for pattern in sensitive_patterns:
                assert pattern not in error_detail, f"Sensitive information leaked: {pattern}"
...
class TestRequestBodyConcurrency:
    ...
    async def test_concurrent_validate_webhook_calls(self):
        ...
        await asyncio.gather(
            *[handler.validate_webhook() for _ in range(5)],
            return_exceptions=True
        )
...
class TestRequestBodyValidation:
    ...
    async def test_cached_body_immutability(self):
        ...
        assert handler._cached_body == original_body or handler._cached_body == original_body + b'modified'
