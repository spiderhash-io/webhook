...
class TestClickHouseAnalyticsConnection:
    ...
    async def test_connect_with_valid_config(self):
        ...
        with patch(
            "src.clickhouse_analytics._validate_connection_host",
            return_value="localhost",
        ), patch("asyncio.get_running_loop") as mock_loop, patch(
            "src.clickhouse_analytics.Client", return_value=mock_client
        ), patch.object(
            analytics, "_ensure_tables", AsyncMock()
        ):
    ...
    async def test_connect_without_password(self):
        ...
        with patch(
            ...
            "src.clickhouse_analytics.Client"
            ...
            call_kwargs = (
                mock_client_class.call_args[1] if mock_client_class.called else {}
            )
...
class TestClickHouseAnalyticsFlushOperations:
    ...
    async def test_flush_logs_without_client(self):
        ...
        await analytics._flush_logs(
            [("id", "webhook", datetime.now(timezone.utc), "payload", "headers")]
        )
    ...
    async def test_flush_stats_with_error(self):
        ...
        buffer = [
            ("id", "webhook", datetime.now(timezone.utc), 10, 1, 2, 3, 4, 5, 6, 7, 8)
        ]
...
class TestClickHouseAnalyticsSaveOperations:
    ...
    async def test_save_log_with_queue(self):
        ...
        with patch("src.utils.CredentialCleaner") as mock_cleaner_class, patch(
            "copy.deepcopy", return_value={"data": "test"}
