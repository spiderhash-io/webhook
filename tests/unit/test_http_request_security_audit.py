...
class TestHTTPRequestParsingEdgeCases:
    ...
    def test_extremely_long_url_rejected(self):
        ...
        config = {"module": "http_webhook", "module-config": {"url": long_url}}
    ...
    def test_url_with_null_byte_rejected(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook\x00evil"},
        }
        with pytest.raises((ValueError, Exception)):
            HTTPWebhookModule(config)
    ...
    def test_url_double_encoding_handled(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/%252e%252e/webhook"},
        }
    ...
    def test_protocol_relative_url_rejected(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "//evil.com/webhook"},
        }
        with pytest.raises((ValueError, Exception)):
            HTTPWebhookModule(config)
    ...
    def test_malformed_url_missing_scheme_rejected(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "example.com/webhook"},
        }
        with pytest.raises((ValueError, Exception)):
            HTTPWebhookModule(config)
...
class TestHostHeaderSecurity:
    ...
    async def test_multiple_host_headers_handled(self):
        ...
        headers = {"Host": "example.com", "host": "evil.com"}  # Case variation
    ...
    async def test_host_header_with_port_manipulation(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com:8080/webhook"},
        }
        module = HTTPWebhookModule(config)
        ...
        assert (
            "evil.com" not in str(sanitized.get("Host", "")).lower()
            or module._sanitize_header_value("evil.com:8080") is not None
        )
...
class TestRequestBodyHandling:
    ...
    async def test_content_length_mismatch_handled(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://httpbin.org/post"},
        }
        module = HTTPWebhookModule(config)
        ...
        headers = {"Content-Length": "10"}  # Mismatch
    ...
    async def test_chunked_transfer_encoding_handled(self):
        ...
        assert (
            "Transfer-Encoding" not in sanitized
            or sanitized.get("Transfer-Encoding") is None
        )
...
class TestConnectionHandling:
    ...
    async def test_connection_timeout_distinct_from_request_timeout(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://httpbin.org/post", "timeout": 5.0},
        }
        module = HTTPWebhookModule(config)
...
class TestHeaderEdgeCases:
    ...
    def test_duplicate_headers_handled(self):
        ...
        headers = {"X-Header": "value1", "x-header": "value2"}  # Case variation
        sanitized = module._sanitize_headers(headers)
    ...
    def test_header_name_case_sensitivity(self):
        ...
        headers = {"X-Test": "value", "x-test": "value2", "X-TEST": "value3"}
        sanitized = module._sanitize_headers(headers)
...
class TestURLEncodingAttacks:
    ...
    def test_percent_encoded_null_byte_blocked(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/webhook%00evil"},
        }
        with pytest.raises((ValueError, Exception)):
            HTTPWebhookModule(config)
    ...
    def test_percent_encoded_path_traversal_blocked(self):
        ...
        config = {
            "module": "http_webhook",
            "module-config": {"url": "http://example.com/%2e%2e/webhook"},
        }
        module = HTTPWebhookModule(config)
        # Should normalize and reject or sanitize
        assert ".." not in module.config.get(
            "url", ""
        ) or "../" not in module.config.get("url", "")
