...
class TestHeaderInjection:
    ...
    async def test_header_type_confusion(self):
        ...
        assert "X-Number" not in sent_headers or isinstance(
            sent_headers.get("X-Number"), str
        )
...
class TestErrorInformationDisclosure:
    ...
    async def test_connection_error_sanitization(self):
        ...
        with patch("stomp.Connection") as mock_conn:
            mock_conn.side_effect = Exception(
                "Connection failed: Access denied for user 'test_user' with password"
            )
            ...
            assert (
                ...
                or "with password" not in error_msg.lower()
            )
    ...
    async def test_publish_error_sanitization(self):
        ...
        mock_client.send.side_effect = Exception(
            "Failed to send: Connection details exposed"
        )
