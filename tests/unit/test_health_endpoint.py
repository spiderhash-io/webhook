...
def app_state_backup():
    ...
    backup = {
        ...
        'webhook_connect_channel_manager': getattr(app.state, 'webhook_connect_channel_manager', None),
    }
    yield backup
    # Restore original state
    app.state.config_manager = backup['config_manager']
    app.state.clickhouse_logger = backup['clickhouse_logger']
    app.state.webhook_connect_channel_manager = backup['webhook_connect_channel_manager']
...
class TestHealthEndpointBasic:
    ...
    def test_health_endpoint_response_structure(self, client):
        ...
        for component, status in data["components"].items():
            ...
            assert status in ["healthy", "unhealthy", "unavailable", "not_configured", "disconnected", "running"]
...
class TestHealthEndpointComponents:
    ...
    def test_health_endpoint_config_manager_unhealthy(self, client, app_state_backup):
        ...
        from src.main import app
        mock_config_manager = Mock()
        mock_config_manager.get_all_webhook_configs.side_effect = Exception("Config error")
        app.state.config_manager = mock_config_manager
        app.state.clickhouse_logger = None
        app.state.webhook_connect_channel_manager = None
    ...
    def test_health_endpoint_webhook_connect_not_configured(self, client, app_state_backup):
...
class TestHealthEndpointErrorHandling:
    ...
    def test_health_endpoint_handles_component_exceptions(self, client, app_state_backup):
        """Test that health endpoint handles exceptions from components gracefully."""
        from src.main import app
        # Make ConfigManager raise exception
        mock_config_manager = Mock()
        mock_config_manager.get_all_webhook_configs.side_effect = Exception("Component error")
        app.state.config_manager = mock_config_manager
        app.state.clickhouse_logger = None
        app.state.webhook_connect_channel_manager = None
...
class TestHealthEndpointSecurity:
    ...
    def test_health_endpoint_response_size(self, client):
        ...
        assert response_size < 2000, "Health endpoint response should be reasonable size"
...
class TestHealthEndpointAppState:
    ...
    def test_health_endpoint_with_all_components_healthy(self, client, app_state_backup):
...
class TestHealthEndpointStatusCodes:
    ...
    def test_health_endpoint_503_when_unhealthy(self, client, app_state_backup):
        ...
        mock_config_manager = Mock()
        mock_config_manager.get_all_webhook_configs.side_effect = Exception("Critical error")
        app.state.config_manager = mock_config_manager
        app.state.clickhouse_logger = None
        app.state.webhook_connect_channel_manager = None
