...
class TestConnectionPoolExhaustion:
        ...
        try:
            ...
            assert any(
                ...
                for keyword in [
                    ...
                    "timeout",
                    "exhausted",
                    "different loop",  # Accept asyncio loop errors in test
                ]
            )
    ...
    async def test_circuit_breaker_activation(self, pool):
        ...
        await asyncio.sleep(
            0.15
        )  # Increased delay to allow circuit breaker to activate
        ...
        assert (
            pool._circuit_breaker_triggered is True
            or pool._last_exhaustion_time is None
        )
    ...
    async def test_circuit_breaker_reset(self, pool):
        ...
        pool._circuit_breaker_triggered = True
        pool._last_exhaustion_time = (
            time.time() - 70
        )  # 70 seconds ago (past reset threshold)
    ...
    async def test_metrics_tracking(self, pool, mock_connection):
        ...
        )  # At least 3 requests (2 successful + 1 timeout attempt)
