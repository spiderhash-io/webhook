...
class TestMQTTTopicPrefixInjection:
    ...
    async def test_topic_prefix_injection_attempts(self):
        ...
        try:
            ...
            assert (
                "invalid topic prefix" in error_msg
                or "dangerous pattern" in error_msg
                or "path traversal" in error_msg
                or "mqtt operation" in error_msg
            )
    ...
    async def test_topic_prefix_with_wildcards(self):
        ...
        config = {
            "module": "mqtt",
            "topic": "events",
            "module-config": {"topic_prefix": "webhook/+/status"},  # Wildcard in prefix
        }
    ...
    async def test_topic_prefix_with_dangerous_patterns(self):
        ...
        for prefix in dangerous_prefixes:
            config = {
                "module": "mqtt",
                "topic": "events",
                "module-config": {"topic_prefix": prefix},
            }
            ...
            try:
                ...
                assert (
                    False
                ), f"Should have raised ValueError for dangerous prefix: {prefix}"
            except (ValueError, Exception) as e:
...
class TestMQTTTasmotaFormatInjection:
    ...
    async def test_tasmota_device_name_injection(self):
        ...
        try:
            ...
            error_msg = str(e).lower()
            assert (
                "dangerous pattern" in error_msg
                or "invalid tasmota device_name" in error_msg
                or "mqtt operation" in error_msg
            )
    ...
    async def test_tasmota_command_injection(self):
        ...
        try:
            ...
            assert (
                "dangerous pattern" in error_msg
                or "invalid tasmota command" in error_msg
                or "mqtt operation" in error_msg
            )
...
class TestMQTTConnectionSecurity:
    ...
    async def test_ssrf_via_host(self):
        ...
        config = {"module": "mqtt", "topic": "test/topic"}
        ...
        for ssrf_host in ssrf_hosts:
            module.connection_details = {"host": ssrf_host, "port": 1883}
    ...
    async def test_port_manipulation(self):
        ...
        for invalid_port in invalid_ports:
            module.connection_details = {"host": "localhost", "port": invalid_port}
...
class TestMQTTQoSManipulation:
    ...
    async def test_qos_type_validation(self):
        ...
        config = {
            "module": "mqtt",
            "topic": "test/topic",
            "module-config": {"qos": "invalid"},  # String instead of int
        }
        ...
        try:
            ...
            error_msg = str(e).lower()
            assert (
                "invalid qos" in error_msg
                or "must be 0, 1, or 2" in error_msg
                or "mqtt operation" in error_msg
            )
    ...
    async def test_qos_out_of_range(self):
        ...
        config = {
            "module": "mqtt",
            "topic": "test/topic",
            "module-config": {"qos": 3},  # Out of range
        }
    ...
    async def test_retained_flag_type_validation(self):
        ...
        config = {
            "module": "mqtt",
            "topic": "test/topic",
            "module-config": {"retained": "true"},  # String instead of bool
        }
...
class TestMQTTMessageFormat:
    ...
    async def test_invalid_message_format(self):
        ...
        config = {
            "module": "mqtt",
            "topic": "test/topic",
            "module-config": {"format": "invalid_format"},  # Invalid format
        }
...
class TestMQTTTopicNameValidation:
    ...
    def test_topic_name_at_max_length(self):
        ...
        config = {"module": "mqtt", "topic": "a" * 1000}  # Valid length
    ...
    def test_topic_name_regex_redos(self):
        ...
        try:
            config = {"module": "mqtt", "topic": complex_name}
            MQTTModule(config)
            assert False, "Should have raised ValueError"
        except ValueError:
