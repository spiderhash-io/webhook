...
class TestS3ObjectKeyInjection:
    ...
    def test_valid_prefixes(self):
        ...
        for prefix in valid_prefixes:
            config = {
                "module": "s3",
                "module-config": {"bucket": "test-bucket", "prefix": prefix},
            }
    ...
    def test_path_traversal_in_prefix_rejected(self):
        ...
        for prefix in traversal_prefixes:
            ...
            with pytest.raises(ValueError) as exc_info:
                S3Module(config)
            assert "path traversal" in str(exc_info.value).lower() or ".." in str(
                exc_info.value
            )
    ...
    def test_valid_filename_patterns(self):
        ...
        for pattern in valid_patterns:
            config = {
                "module": "s3",
                "module-config": {"bucket": "test-bucket", "filename_pattern": pattern},
            }
            module = S3Module(config)
    ...
    def test_control_characters_rejected(self):
        ...
        for pattern in control_chars:
            ...
            error_msg = str(exc_info.value).lower()
            assert any(
                keyword in error_msg for keyword in ["control", "forbidden", "invalid"]
            )
    ...
    def test_empty_values_rejected(self):
        ...
        config = {
            "module": "s3",
            "module-config": {"bucket": "test-bucket", "prefix": ""},
        }
        with pytest.raises(ValueError) as exc_info:
            S3Module(config)
        ...
        config = {
            "module": "s3",
            "module-config": {"bucket": "test-bucket", "filename_pattern": ""},
        }
        with pytest.raises(ValueError) as exc_info:
            S3Module(config)
        assert "empty" in str(exc_info.value).lower()
    ...
    def test_length_limits(self):
        ...
        config = {
            "module": "s3",
            "module-config": {"bucket": "test-bucket", "prefix": long_prefix},
        }
        with pytest.raises(ValueError) as exc_info:
            S3Module(config)
    ...
    def test_whitespace_handling(self):
        ...
        config = {
            "module": "s3",
            "module-config": {"bucket": "test-bucket", "prefix": "  webhooks  "},
        }
        module = S3Module(config)
    ...
    def test_placeholder_replacement_safety(self):
        ...
        )  # Replace colons for safety
        filename = module_timestamp._validated_filename_pattern.replace(
