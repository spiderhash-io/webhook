...
class TestDigestAuth:
    ...
    async def test_digest_auth_missing_required_parameter(self):
        ...
        # Missing nonce
        auth_header = 'Digest username="testuser", realm="Test Realm", uri="/webhook/test"'
    ...
    async def test_digest_auth_special_characters_in_credentials(self):
        ...
        response = generate_digest_response(
            "user@domain.com", "p@ss:w0rd!", "Test Realm", "POST", uri, nonce, nc, cnonce
        )
    ...
    async def test_digest_auth_sql_injection_attempt(self):
        ...
        assert "Invalid digest auth username" in message or "Invalid digest auth response" in message
