...
class TestSQLInjectionTableColumnNames:
    ...
    def test_table_name_null_byte_injection(self):
        ...
        config = {"connection_details": {}, "module-config": {"table": "table\x00name"}}
        with pytest.raises(ValueError):
            PostgreSQLModule(config)
    ...
    async def test_schema_field_name_validation(self):
        ...
        config = {
            ...
            "module-config": {
                ...
                "schema": {
                    "fields": {
                        "field\x00name": {"type": "string", "column": "valid_column"}
                    }
                },
            },
        }
        ...
        mock_pool.acquire = MagicMock(
            return_value=MagicMock(
                __aenter__=AsyncMock(
                    return_value=MagicMock(
                        execute=AsyncMock(), fetchval=AsyncMock(return_value=1)
                    )
                ),
        ...
        with patch(
            "asyncpg.create_pool", side_effect=AsyncMock(return_value=mock_pool)
        ):
            # Security: Field name with null byte should be rejected during setup
            with pytest.raises(
                ValueError, match="Field name contains dangerous characters"
            ):
                await module.setup()
    ...
    async def test_index_name_injection(self):
        ...
        config = {
            ...
            "module-config": {
                ...
                "storage_mode": "json",
                "schema": {
                    "indexes": {"idx'; DROP TABLE users; --": {"columns": ["col1"]}}
                },
            },
        }
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Invalid table name format|SQL keyword|dangerous pattern",
            ):
                await module.setup()
    ...
    async def test_index_column_injection(self):
        ...
        config = {
            ...
            "module-config": {
                ...
                "schema": {
                    "indexes": {
                        "valid_index": {"columns": ["col'; DROP TABLE users; --"]}
                    }
                },
            },
        }
...
class TestUpsertKeyInjection:
    ...
    async def test_upsert_key_json_path_injection(self):
        ...
        with pytest.raises(
            ValueError, match="upsert_key contains dangerous characters"
        ):
            PostgreSQLModule(config)
    ...
    async def test_connection_string_type_confusion(self):
        ...
        for invalid_conn_str in invalid_types:
            ...
            if isinstance(invalid_conn_str, str):
                ...
                with pytest.raises(
                    ValueError, match="Invalid connection string format"
                ):
                    await module.setup()
            else:
                # Non-string - should fail type check
                with pytest.raises(
                    ValueError, match="Connection string must be a string"
                ):
                    await module.setup()
...
class TestPoolSizeDoS:
    ...
    async def test_pool_min_size_negative(self):
        ...
        try:
            with patch(
                "asyncpg.create_pool", side_effect=AsyncMock(return_value=AsyncMock())
            ):
                await module.setup()
        except (ValueError, Exception):
    ...
    async def test_pool_size_type_confusion(self):
        ...
        for invalid_size in invalid_sizes:
            ...
            try:
                with patch(
                    "asyncpg.create_pool",
                    side_effect=AsyncMock(return_value=AsyncMock()),
                ):
                    await module.setup()
    ...
    async def test_relational_mode_field_value_injection(self):
        ...
        config = {
            ...
            "module-config": {
                "table": "test_table",
                "storage_mode": "relational",
                "schema": {"fields": {"field1": {"type": "string", "column": "col1"}}},
            },
            "_webhook_id": "test",
        }
        ...
        if call_args[0]:  # positional args
            ...
            assert "'; DROP TABLE users; --" in args or any(
                "'; DROP TABLE users; --" in str(arg) for arg in args
            )
...
class TestConfigurationSecurity:
    ...
    def test_storage_mode_type_validation(self):
        ...
        assert module.storage_mode in ["json", "relational", "hybrid"] or isinstance(
            module.storage_mode, (str, int)
        )
    ...
    def test_upsert_type_validation(self):
        ...
        assert isinstance(module.upsert, bool) or module.upsert in [
            True,
            False,
            "true",
            "false",
    ...
    def test_include_headers_type_validation(self):
        ...
        assert isinstance(module.include_headers, bool) or module.include_headers in [
            True,
            False,
            "yes",
            "no",
