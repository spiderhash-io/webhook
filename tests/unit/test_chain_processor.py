...
class TestChainProcessor:
    ...
    def mock_webhook_config(self):
        ...
        return {"data_type": "json", "authorization": "Bearer token"}
    ...
    def detailed_chain(self):
        ...
        return [
            {"module": "log", "connection": "local"},
            {"module": "save_to_disk", "module-config": {"path": "/tmp"}},
        ]

    @pytest.mark.asyncio
    async def test_sequential_execution_success(
        self, mock_webhook_config, simple_chain
    ):
    ...
    async def test_sequential_execution_with_failure_stop(self, mock_webhook_config):
        ...
        with patch("src.chain_processor.ModuleRegistry") as mock_registry:
            ...
            processor = ChainProcessor(
                chain=chain,
                chain_config={"execution": "sequential", "continue_on_error": False},
                webhook_config=mock_webhook_config,
            )
    ...
    async def test_parallel_execution_with_failure(self, mock_webhook_config):
        ...
        with patch("src.chain_processor.ModuleRegistry") as mock_registry:
            ...
            processor = ChainProcessor(
                chain=chain,
                chain_config={"execution": "parallel", "continue_on_error": True},
                webhook_config=mock_webhook_config,
            )
    ...
    async def test_build_module_config(self, mock_webhook_config, detailed_chain):
        ...
        processor = ChainProcessor(
            chain=detailed_chain, chain_config={}, webhook_config=mock_webhook_config
        )
    ...
    def test_build_module_config_without_connection_config(self, mock_webhook_config):
        ...
        chain_item = {"module": "log", "module-config": {}}
    ...
    def test_build_module_config_missing_connection(self, mock_webhook_config):
        ...
        connection_config = {
            "postgres_local": {"type": "postgresql", "host": "localhost"}
        }
        ...
        assert (
            "connection_details" not in config or config.get("connection_details") == {}
        )
    ...
    async def test_module_instantiation_with_pool_registry(self, mock_webhook_config):
        ...
        kafka_config = {
            "module": "kafka",
            "topic": "test_topic",
            "connection_details": {"bootstrap_servers": "localhost:9092"},
        }
