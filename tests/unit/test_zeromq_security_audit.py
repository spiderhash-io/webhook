...
class TestEndpointInjection:
    ...
    def test_endpoint_type_confusion(self):
        ...
        config = {"connection_details": {}, "module-config": {"endpoint": None}}
    ...
    def test_endpoint_dangerous_schemes(self):
        ...
        for scheme in dangerous_schemes:
            config = {"connection_details": {}, "module-config": {"endpoint": scheme}}
    ...
    def test_endpoint_control_characters(self):
        ...
        for endpoint in invalid_endpoints:
            config = {"connection_details": {}, "module-config": {"endpoint": endpoint}}
...
class TestErrorInformationDisclosure:
    ...
    async def test_socket_creation_error_sanitization(self):
        ...
        mock_zmq.asyncio.Context.side_effect = Exception(
            "Failed to bind: Address already in use tcp://example.com:5555"
        )
        ...
        assert (
            "Address already in use" not in error_msg or "Processing error" in error_msg
        )
    ...
    async def test_publish_error_sanitization(self):
        ...
        module = ZeroMQModule(config)
        mock_socket = AsyncMock()
        mock_socket.send_string.side_effect = Exception(
            "Socket not connected: tcp://example.com:5555"
        )
        module.socket = mock_socket
        ...
        # Should not expose internal details
        assert (
            "Socket not connected" not in error_msg or "Processing error" in error_msg
        )
