...
class TestWebSocketIntegration:
    ...
    async def test_websocket_ssrf_prevention_metadata_endpoint_blocked(self):
        ...
        for url in metadata_urls:
            config = {"module": "websocket", "module-config": {"url": url}}
            with pytest.raises(
                ValueError, match="metadata|link-local|not allowed|security"
            ):
                WebSocketModule(config)
    ...
    async def test_websocket_url_format_validation(self):
        ...
        config_invalid = {"module": "websocket", "module-config": {"url": "not-a-url"}}
        with pytest.raises(ValueError, match="scheme|not allowed"):
            WebSocketModule(config_invalid)

        # Test no hostname
        config_no_host = {"module": "websocket", "module-config": {"url": "ws://"}}
        ...
        try:
            config_empty = {"module": "websocket", "module-config": {"url": ""}}
