...
class TestRedisIntegration:
    ...
    async def test_redis_publish_webhook(
        ...
        payload = {"test": "redis_integration", "timestamp": "2024-01-01T00:00:00Z"}
        ...
        try:
            message = await asyncio.wait_for(
                pubsub.get_message(timeout=2.0), timeout=3.0
            )
            if message and message["type"] == "message":
                assert message["channel"] == test_channel
                # Message should contain the payload
                assert (
                    "test" in message["data"] or "redis_integration" in message["data"]
                )
