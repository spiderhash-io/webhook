...
class TestChainPostgresKafkaE2E:
    ...
    async def test_chain_postgres_then_kafka_full_flow(
        ...
        assert "connection details not found" not in response_text, (
            "Connection details should be injected from connections.json"
        )
        assert "unexpected keyword argument 'pool_registry'" not in response_text, (
        ...
        assert row is not None, f"Data with event_id '{test_event_id}' should exist in PostgreSQL"
        ...
        try:
            ...
            assert msg.key.decode('utf-8') == test_event_id, f"Expected key '{test_event_id}', got '{msg.key.decode('utf-8')}'"
    ...
    async def test_chain_connection_details_injection(self, client):
        ...
        assert "connection_details" in postgres_module_config, (
        ...
        assert "connection_details" in kafka_module_config, (
            "connection_details should be injected from connection_config"
        )
        assert kafka_module_config["connection_details"]["type"] == "kafka"
        assert kafka_module_config["connection_details"]["bootstrap_servers"] == "redpanda:9092"
        ...
        try:
            postgres_module = PostgreSQLModule(postgres_module_config, pool_registry=None)
