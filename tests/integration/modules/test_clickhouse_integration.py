...
class TestClickHouseIntegration:
    ...
    async def test_clickhouse_table_creation(self):
        ...
        response = httpx.post(f"{CLICKHOUSE_HTTP_URL}/", data=create_query, timeout=5.0)
    ...
    async def test_clickhouse_webhook_logging(
        self, http_client: httpx.AsyncClient, test_webhook_id: str, test_auth_token: str
    ):
        ...
        response = httpx.get(
            f"{CLICKHOUSE_HTTP_URL}/", params={"query": check_table_query}, timeout=5.0
        )
    ...
    async def test_clickhouse_data_insert_query(self):
        ...
        response = httpx.get(
            f"{CLICKHOUSE_HTTP_URL}/", params={"query": select_query}, timeout=5.0
        )
        assert response.status_code == 200
