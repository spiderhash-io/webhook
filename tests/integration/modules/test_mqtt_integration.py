...
class TestMQTTIntegration:
    ...
    async def test_mqtt_topic_validation(self):
        ...
        valid_config = {
            ...
            "connection_details": {"host": MQTT_HOST, "port": MQTT_PORT},
        }
    ...
    async def test_mqtt_retained_messages(self, mqtt_consumer):
        ...
        config = {
            ...
            "topic": test_topic,
            "connection_details": {"host": MQTT_HOST, "port": MQTT_PORT},
            "module-config": {"retained": True, "qos": 1},
        }
            ...
            async with mqtt_consumer.messages() as messages_stream:
                for _ in range(5):
                    message = await asyncio.wait_for(
                        messages_stream.__anext__(), timeout=5.0
                    )
                    received_data = json.loads(message.payload.decode("utf-8"))
    ...
    async def test_mqtt_connection_error_handling(self):
        ...
        config = {
            "module": "mqtt",
            "topic": "test_topic",
            "connection_details": {"host": "invalid_host", "port": 1883},
        }
        ...
        with pytest.raises(
            Exception,
            match="MQTT|connection|operation|name resolution|refused|Temporary failure|Errno",
        ):
    ...
    async def test_mqtt_topic_prefix(self, mqtt_consumer):
        ...
        config = {
            ...
            "topic": "events",
            "connection_details": {"host": MQTT_HOST, "port": MQTT_PORT},
            "module-config": {"topic_prefix": "webhook/prefix"},
        }
