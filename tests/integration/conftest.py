...
async def services_ready(docker_services_available) -> bool:
    ...
    redis_ready = await wait_for_service(
        check_redis_health, timeout=30.0, service_name="Redis"
    )
    rabbitmq_ready = await wait_for_service(
        check_rabbitmq_health, timeout=30.0, service_name="RabbitMQ"
    )
    clickhouse_ready = await wait_for_service(
        check_clickhouse_health, timeout=30.0, service_name="ClickHouse"
    )
    kafka_ready = await wait_for_service(
        check_kafka_health, timeout=30.0, service_name="Kafka/Redpanda"
    )
...
async def api_server_ready(services_ready) -> bool:
    ...
    api_ready = await wait_for_service(
        check_api_health, timeout=10.0, service_name="FastAPI Server"
    )
...
async def authenticated_client(
    ...
    http_client.headers.update({"Authorization": f"Bearer {TEST_AUTH_TOKEN}"})
    yield http_client
