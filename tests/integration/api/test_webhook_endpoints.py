...
class TestWebhookEndpoints:
    ...
    async def test_webhook_without_authentication(
        self, http_client: httpx.AsyncClient, test_webhook_id: str
    ):
        """Test webhook endpoint without authentication."""
        payload = {"test": "data"}

        response = await http_client.post(f"/webhook/{test_webhook_id}", json=payload)
    ...
    async def test_webhook_nonexistent(
        self, http_client: httpx.AsyncClient, test_auth_token: str
    ):
